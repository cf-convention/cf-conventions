...
This section first makes some general points about CF calendars, then describes the calendar corresponding to each of the possible values for the **`calendar`** attribute; this information is summarised in <<table-calendars, Table 4.1>> at the end of the section.
It is recommended that the calendar of the time coordinate variable be explicitly specified by its **`calendar`** attribute (rather than relying on the default).

A calendar defines a set of valid datetimes and their order.
...
Moreover, a given datetime in the **`utc`** calendar represents an instant that is later than the same datetime in the **`tai`** calendar.
+
...
| **`standard`** | 365/366 | Gregorian | Gregorian | unknown | 0001-01-01 | +&#x221E; | no (climatology)
| **`julian`** | 365/366 | Gregorian | Julian | no | 0001-01-01 | +&#x221E; | no (climatology)
