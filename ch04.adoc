...
The most commonly used of these units strings (and their symbols) are **`day`** (**`d`**), **`hour`** (**`hr`**, **`h`**), **`minute`** (**`min`**) and **`second`** (**`sec`**, **`s`**).
Plural forms are also acceptable.
...
It must include the date, which may be followed by the time, and optionally a time zone offset.
...
Other than **`Z`** for zero offset, the time zone offset __T__ must be in one of the following four numeric formats, each of which may optionally be prefixed by a positive sign **`+`** or negative sign **`-`**, the default sign being positive:
...
If __T__ is **`Z`** or begins with **`+`** or **`-`**, the space before __T__ may be omitted.
...
This is the default calendar when there is no **`calendar`** attribute, except when the attribute `month_lengths` is present (see <<explicit_calendar>>).
...
Datetimes in this range must not be used as reference in **`units`**, and the interval between the datetimes 1582-10-15 0:0:0 and 1582-10-4 0:0:0 is exactly 1 day.
...
This instant has the datetime 1972-01-01 00:00:10 (10 seconds greater) in the **`tai`** calendar.
...
The difference between the two datetimes (**`tai`** minus **`utc`**) for a given instant of time is the net number of leap seconds introduced since 1958-01-01.
+
