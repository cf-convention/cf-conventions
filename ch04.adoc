...
Plural forms are also acceptable.
Any unit may optionally have one of the decimal prefixes for multiples and submultiples (<<table-supported-units, Table 3.1>>), but these are uncommon except for submultiples of **`second`** e.g. **`millisecond`** or **`ms`**.
...
* __S__ is second, which may be integer or floating point (see <<leap-seconds>> regarding __S__,
