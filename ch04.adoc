...
The lengths of the months defined by the Julian and Gregorian calendars (12 months in the year, 31 days in January, etc.) are used in all calendars except **`360_day`**, **`none`** (see <<none-calendar>>) and explicitly defined calendars (see <<explicit-calendar>>).
...
* if you are using an existing observational dataset for dates since 1972 with time coordinates in the **`standard`** or **`utc`** calendar, which treat them differently.
Later paragraphs of this section give further information about these cases.

* if you are using an observational datasets with time coordinates in the **`tai`** calendar.
These will not give UTC when converted to datetimes.
...
It is recommended to use the **`proleptic_gregorian`** or the **`utc`** calendar in preference to the **`standard`** calendar in this case.
<<leap-seconds>> compares these calendars' treatment of leap seconds in detail.
You need the **`utc`** calendar to preserve precision to the second in all datetimes and differences between time coordinates.
However, the choice of **`utc`** requires both the writer and the user of the dataset to have software that takes leap seconds into account for converting between datetimes and elapsed times.
Most software ignores leap seconds.

* if you are writing a model dataset following the real-world Gregorian calendar for dates since 1972.
It is recommended to use the **`proleptic_gregorian`** calendar in preference to the **`standard`** calendar in this case, in order to be completely clear to the data user that there are no leap seconds.
...
See <<leap-seconds>> for further discussion.
...
If you are producing real-world datasets with datetimes in UTC, and if it's important for your time coordinates to be accurate to the second, the **`utc`** calendar is recommended.
