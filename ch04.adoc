...
==== Calendar
A time coordinate value is a number which represents a date-time as a count.
...
In order to calculate a time coordinate value from a date-time, or the reverse, one must know the **`units`** of the time coordinate variable (containing the counting unit and the reference date-time) and the calendar.
