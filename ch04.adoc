...
A timestamp in the **`utc`** calendar represents an instant that is earlier than the same timestamp in the **`tai`** calendar, by the amount of leap seconds introduced since the start of 1958.
See <<leap-seconds>>.
...
A timestamp in the **`tai`** calendar represents an instant that is later than the same timestamp in the **`utc`** calendar, by the amount of leap seconds introduced since the start of 1958. 
See <<leap-seconds>>.
...
The **`standard`**, **`proleptic_gregorian`**, and **`julian`** calendars defined in <<calendar>> are special in that they can be used either __with__ or __without__ leap seconds.
For these calendars, there are the following cases:

1. **The calendar may be used for a timeline in which leap seconds do not exist.**
...
2. **The calendar may be used for a timeline in which leap seconds exist.**
...
Although the timeline contains timestamps for leap seconds in this case, such timestamps are not counted when calculating time coordinates.
Therefore the difference between two time coordinates does __not__ include any leap seconds which occurred between the instants they represent.
...
3. **The calendar may be used for a timeline in which leap seconds exist, but some or all leap seconds might not have not been correctly accounted for.**
...
For many applications, any inaccuracies introduced by incorrectly assuming which of these possibilities applies are likely to be negligible, but there are some applications where they are important.
...
variables:
  float time1 ;
    ...
    time1:units = "seconds since 2016-12-31" ;
  ...
  float time2 ;
    ...
    time2:units = "seconds since 2016-12-31" ;
