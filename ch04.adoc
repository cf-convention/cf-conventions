...
pass:q[<del><span style="color: red">Leap seconds should be considered:`</span></del>]

* pass:q[<del><span style="color: red">if you are using an existing observational dataset for dates since 1972 with time coordinates in the **`standard`**, `proleptic_gregorian or **`utc`** calendar, which treat them differently.
Later paragraphs of this section give further information about these cases.`</span></del>]

* pass:q[<del><span style="color: red">if you are using an existing observational dataset with time coordinates in the **`tai`** calendar.
These do not give UTC when converted to datetimes.
See the later paragraph about the **`tai`** calendar.`</span></del>]

##If you are producing a dataset which follows the Gregorian calendar, the recommended choice of CF **`calendar`** is as follows:##

* ##Use the **`utc`** calendar for observational data## pass:q[<del><span style="color: red">if you are writing an observational dataset for dates since 1972 using datetimes from a clock or data logging device whose time and date are related to civil time (e.g. from clocks based on UTC or computers obtaining time signals from an NTP server)</span></del>]##for datetimes since 1972 based on UTC, if it is necessary to preserve precision to the second in both datetimes and time intervals.##
...
Leap seconds do not need to be considered pass:q[<del><span style="color: red">for model-generated data in any model calendar</span></del>]##in the **`360_day`**, **`365_day`** and **`366_day`** calendars##.
