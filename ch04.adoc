...
double time(time) ;
  ...
  time:calendar = "standard" ; // recommended
...
To avoid mistakes and confusion, we therefore recommend that these units should not be used in the **`utc`** calendar.
...
Other than **`Z`** for zero offset, the time zone offset __T__ must be in one of the following four signed numeric formats, where &pm; stands for the positive sign **`+`** or the negative sign **`-`**, and __n__ stands for a single digit:

* &pm;__H__, the hour alone, of one or two digits e.g. **`-6`**, **`-0`**, **`+0`**, **`+11`**, which is sufficient for many time zones.

* &pm;__H__:__M__, where __H__ is hour and __M__ minute, each of one or two digits, and separated by a colon **`:`**, e.g. **`+5:30`**.

* &pm;__nnnn__, where the first pair of digits are the hours and the second the minutes e.g. **`+0530`**.

* &pm;__nnn__, where the first digit is the hour (0--9) and the others are the minutes e.g. **`530`**.
...
It is recommended that the calendar be explicitly specified by the **`calendar`** attribute of the time coordinate variable (rather than relying on the default).
First, this section gives an overview of CF calendars.
Second, it describes the calendar corresponding to each of the possible values for the **`calendar`** attribute; this information is summarised in <<table-calendars, Table 4.1>> at the end of the section.

In a given calendar, each valid datetime identifies a particular instant in the continuous physical dimension of time.
The reference datetime string in the **`units`** must be valid for the calendar, and identifies a particular instant according to the calendar.
A given datetime can identify a different instant in different calendars.
...
In the **`julian`** and the **`standard`** calendar, dates in years before year 0 (i.e. before 0-1-1 0:0:0) are not allowed, and the year in the reference datetime of the units must not be negative.
...
Owing to variations in Earth rotation, positive or negative leap seconds are inserted into UTC in order to keep it close to mean solar time at 0 **`degrees_east`** i.e. the time zone with the default (zero) time zone offset in UDUNITS and CF (see <<time-coordinate-units>>).
When a single positive leap second is introduced at the end of a minute, that minute contains 61 seconds.
The net number of leap seconds added to UTC between 1972-1-1 and 2025-1-1 is 27.
Leap seconds should be considered when processing datetimes and time-coordinates in the **`standard`** and **`utc`** calendars, which treat them differently.
In the other CF calendars, leap seconds never need to be considered.

For observational or model-generated data that follows the Gregorian calendar and UTC (after 1972) including leap seconds, if precision to the second is important, the **`utc`** calendar is recommended in preference to the **`standard`** calendar.
For model-generated data following the Gregorian calendar without leap seconds, the **`proleptic_gregorian`** calendar is recommended in preference to the **`standard`** calendar.
For further discussion of this issue, see <<leap-seconds>>.

The possible values for the **`calendar`** attribute and the definitions of the calendars are as follows:
...
Datetimes in this range must not be used as reference in **`units`**, and the interval between the datetimes 1582-10-4 0:0:0 and 1582-10-15 0:0:0 is exactly 1 day.
...
For producing new datasets, where precision to the second is important, it is recommended to use the **`utc`** calendar for data (whether observed or simulated) that follows the real-world calendar according to UTC including leap seconds, and the **`proleptic_gregorian`** calendar for model-generated data that follows the Gregorian calendar without leap seconds.
For further discussion of this issue, see <<leap-seconds>>.
...
The current definition of UTC came into force at 1972-01-01 00:00.
This instant has the datetime 1972-01-01 00:00:10 in the **`tai`** calendar.
...
Earlier datetimes than 1972-01-01 00:00 are not allowed in the **`utc`** calendar.
...
The difference between the two datetimes (**`tai`** minus **`utc`**) for a given instant of time is 10 seconds plus the net number of leap seconds introduced since 1972-01-01.
+
...
**`tai`**:: A Gregorian calendar __without__ leap seconds that is based on International Atomic Time (TAI), which began on 1958-01-01 0:0:0.
...
[[table-calendars]]
.List of defined values for the **`calendar`** attribute, each with a summary of the key characteristics of the calendar it selects
[options="header",caption="Table 4.1. "]
|===============
| **`calendar`** | Days in year | Months | Leap days | Leap seconds | First date | Last date | Year 0
| **`standard`** | 365/366 | Gregorian | Gregorian | unknown | 0001-01-01 | +&#x221E; | climatology
| **`julian`** | 365/366 | Gregorian | Julian | no | 0001-01-01 | +&#x221E; | climatology
| **`proleptic_gregorian`** | 365/366 | Gregorian | Gregorian | no | -&#x221E; | +&#x221E; | yes
| **`no_leap`**/**`365_day`** | 365 | Gregorian | never | no | -&#x221E; | +&#x221E; | yes
| **`all_leap`**/**`366_day`** | 366 | Gregorian | every year | no | -&#x221E; | +&#x221E; | yes
| **`360_day`** | 360 | 12 x 30 days | never | no | -&#x221E; | +&#x221E; | yes
| **`utc`** | 365/366 | Gregorian | Gregorian | yes | 1972-01-01 | present day | no
| **`tai`** | 365/366 | Gregorian | Gregorian | no | 1958-01-01 | +&#x221E; | no
|===============
In this table, "/" means "or", "-&#x221E;" means "any date (provided it is earlier than the "Last date", if any)" and "+&#x221E;" means "any date (provided it is later than the "First date", if any)". In the "Leap days" column, "Gregorian" and "Julian" indicate the rules for determing which years are leap years, in which a leap day is added to February. In the "Year 0" column, "yes" and "no" respectively indicate that there are, or are not, valid dates in year 0 in this calendar, while "climatology" indicates that there are no valid dates in year 0, but it may be used to denote a climatology (however, this use of year 0 is deprecated; see <<climatological-statistics>>).


[[leap-seconds, Section 4.4.4, "Leap Seconds"]]
==== Leap Seconds

This section describes the treatment of leap seconds in CF in more detail than <<calendar>>.
