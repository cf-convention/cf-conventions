...
Plural forms are also acceptable.
Any unit may optionally have one of the decimal prefixes for multiples and submultiples (<<table-supported-units>>), but these are uncommon except for submultiples of **`second`** e.g. **`millisecond`** or **`ms`**.
...
The default for _Z_ is zero offset, which may also be explicitly indicated in any of the numeric formats for _Z_ defined below, or by the letter `Z`, sometimes referred to as "Zulu Time".
We suggest that a zero offset be stated explicitly to avoid any confusion in any situation where omitting it might be misunderstood as indicating local time.
...
Other than `Z` for zero offset, the time zone offset _Z_ must be in one of the following four numeric formats, each of which may optionally be prefixed by a positive sign `+` or negative sign `-`, the default sign being positive:

** _H_, the hour alone, of one or two digits e.g. **`-6`**, **`2`**, **`+11`**, which is sufficient for many time zones.

** __H__:__M__, where _H_ is hour and _M_ minute, each of one or two digits, and separated by a colon `:`, e.g. **`5:30`**.
...
If _Z_ is `Z` or begins with `+` or `-`, the space before _Z_ may be omitted.
...
In this calendar, the second is the only recommended unit of measure in the **`units`** of the time coordinate variable (denoted by any string allowed in UDUNITS, optionally with one of the prefixes of <<table-supported-units>>).
...
CF version 1.12 introduced possible values of the form "**`leap_seconds:`** _value" for the **`units_metadata`** attribute, in order to specify the treatment of leap seconds in the **`standard`**, **`julian`** and **`proleptic_gregorian`** calendars.
