...
[[parameterized-vertical-coordinate, Section 4.3.3, "Parameterized Vertical Coordinate"]]
==== Parameterized Vertical Coordinate
...
in <<parameterized-v-coord>>. The definition provides a mapping between the
...
In this example the **`standard_name`** value **`atmosphere_sigma_coordinate`** identifies the following definition from <<parameterized-v-coord>> which specifies how to compute pressure at gridpoint **`(n,k,j,i)`** where **`j`** and **`i`** are horizontal indices, **`k`** is a vertical index, and **`n`** is a time index:
----
