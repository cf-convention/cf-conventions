...
For producing new datasets, where precision to the second is important, it is recommended to use the **`utc`** calendar for data that follows the real-world calendar including leap seconds, and the **`proleptic_gregorian`** calendar for model-generated data that follows the Gregorian calendar without leap seconds.
