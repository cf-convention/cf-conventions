...
The grid resolution is controlled by the zoom_level, starting from 0. At a given zoom level, the total number of cells is calculated as:
\text{number of cells} = 12 \times 4^{\text{zoom\_level}}  See <<GHB05>> for the calculation of center coordinates and cell boundaries coordinates. 
