...
|  The name of the geodetic (horizontal) datum, which corresponds to the 
     ...
     https://cf-pcmdi.llnl.gov/trac/wiki/Cf2CrsWkt (horiz_datum.csv, OGC_DATUM_NAME column)
     *FIX ALL URLS.  The referenced url should be both moved and updated.*  and are 
...
| The name of the prime meridian associated with the geodetic datum. Valid 
    names are given in https://cf-pcmdi.llnl.gov/trac/wiki/Cf2CrsWkt
...
| The name of the reference ellipsoid. Valid names are given in 
   https://cf-pcmdi.llnl.gov/trac/wiki/Cf2CrsWkt  (ellipsoid.csv). 
