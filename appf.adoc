...
The total number of cells on the sphere is `12 * (4 ^ refinement_level)`.
The lowest allowed refinement level is `0`, giving 12 cells globally.
The algorithm that assigns cell locations to **`nested`** or **`ring`** indices at a given refinement level is defined in <<GHB05>>. In this algorithm, a cell with index `0`, at any refinment level, always has its southern vertex on the meridian at 45 degrees east.
+
...
+
An index according to the **`nuniq`** indexing scheme can be computed from a refinement level and a **`nested`** index with the formula `index_nuniq = index_nested + 4 ^ (1 + refinement_level)` <<RH15>>.
+
An index according to the **`zuniq`** indexing scheme can be computed from a refinement level and a **`nested`** index with the formula `index_zuniq = (2 * index_nested + 1) * 4 ^ (29 - refinement_level)` <<K24>>.
+
