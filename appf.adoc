...
__Map parameters:__::
* **`indexing_scheme`**
* **`refinement_level`** - Mandatory for `indexing_scheme` "nested" or "ring", must be omitted for "nuniq" or "zuniq".

__Map coordinates:__::
The map uses a discrete 1-dimensional "index" coordinate, which uses the `standard_name` `"healpix_index"`, and can additionally be identified through the `grid_mapping` attribute.
The HEALPix index coordinates may be stored in a coordinate, auxiliary, or scalar coordinate variable. Indexing schemes are optimized to allow applications efficient data access when the HEALPix indices are ordered strictly monotonically. Therefore it is recommended to store strictly monotonic HEALPix indices in a coordinate variable, which allows an application to assess their monotonicity without expensive checking. Note that it may be possible to encode consecutive HEALPix indices in a compact way with a <<compression-by-coordinate-subsampling, tie point coordinate variable>>.
If tie-point coordinates are used, take special care of the required precision.

+
The coordinate system is defined by discrete cells on a sphere, arranged on a HEALPix grid.
...
Cells are indexed using either the nested, ring, nuniq or zuniq indexing scheme, the indices are stored in the index coordinate, which can be identified through the standard name `healpix_index`, or the `grid_mapping` attribute.

+
For "nested" and "ring" scheme, the grid resolution is controlled by the refinement level parameter **`refinement_level`** <<GHB05>>.
0 is the coarsest, the total number of cells on the sphere is `12 * 4 ^ refinement_level`.
For the "nuniq" and "zuniq" schemes, the resolution of a cell is implied by the index, and `refinement_level` must be omitted.

+
An index according to the `nuniq` indexing scheme can be computed from the `refinement_level` and a `nested` index as follows <<RH15>>:

+
`index_nuniq = index_nested + 4 ^ (1 + refinement_level)`

+
The `zuniq` indexing scheme depends on the number of bits used by the integer type.
This specification assumes 64 bit integers for all `zuniq` indices.
An index according to the `zuniq` indexing scheme can be computed from the `refinement_level` and a `nested` index as follows <<K24>>:

+
`index_zuniq = (2 * index_nested + 1) * 4 ^ (29 - refinement_level)`

+
The value `29` is the maximum representable `refinement_level` and is computed from the number of bits as `(n_bits - 4 - 1) // 2`.
...
| **`indexing_scheme`** | S
| Identifier of the HEALPix cell indexing scheme, either `nested`, `ring`, `nuniq` or `zuniq`. See <<healpix>> for more detailed definitions.
