...
This means that a given viewing angle described using thsi model is the result of matrix multiplications, which is not commutative, so that order of operations is essential in achieving accurate results.
...
+
Notes on using the PROJ software for computing the mapping and explanatory diagrams may be found at link:$$http://proj.org/operations/projections/geos.html$$[on the PROJ website].
