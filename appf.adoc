...
__Map parameters:__::
* **`indexing_scheme`** - This parameter is optional (default is `nested`)
* **`refinement_level`** - This parameter defines the HEALPix refinement level (0 == coarsest, optional if `indexing_scheme` is "nuniq" or "zuniq")
...
The coordinate can be any kind of coordinate, apart from primary or auxiliary coordinates, especially a tie-point coordinate can be used to encode contiguous blocks in index space.
...
See <<GHB05>> for the calculation of center coordinates and cell boundaries coordinates.
...
For "nested" and "ring" scheme, the grid resolution is controlled by the refinement level parameter **`refinement_level`**, for "nuniq" and "zuniq", the resolution of a cell is implied by the index.
...
TODO: add computation for `zuniq`. This seems to depend on `DEPTH_MAX`, which seems to depend on the number of available bits. How do we (formally) define DEPTH_MAX?

At a given refinement level, the total number of cells corresponds to 12 \times 4^{\text{refinement\_level}}
...
| **`indexing_scheme`** | S
| Identifier of the HEALPix cell ordering, either `nested`, `ring`, `nuniq` or `zuniq`. See <<GHB05>> TODO: we need referenes or definitions for `nuniq` and `zuniq`.
