...
|  The name of the geodetic (horizontal) datum, which corresponds to the 
     ...
     names and their associated parameters are given in link:$$ https://cf-pcmdi.
     llnl.gov/trac/wiki/Cf2CrsWkt (horiz_datum.csv, OGC_DATUM_NAME column) and are 
     ...
     link:$$ http://home.gdal.org/projects/opengis/wktproblems.html for more details). 
     Corresponds to a OGC WKT DATUM node name towgs84 N .  This indicates a list of up 
...
| The name of the prime meridian associated with the geodetic datum. Valid 
    names are given in    link:$$ https://cf-pcmdi.llnl.gov/trac/wiki/Cf2CrsWkt  
...
| The name of the reference ellipsoid. Valid names are given in 
   link:$$ https://cf-pcmdi.llnl.gov/trac/wiki/Cf2CrsWkt  (ellipsoid.csv). 
...
Notes: (1) The various *_name attributes are optional but recommended when 
known as they allow for a better description and interoperability with WKT 
definitions. (2) reference_ellipsoid_name, prime_meridian_name, 
