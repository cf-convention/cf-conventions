...
__Map coordinates:__:: The x (abscissa) and y (ordinate) rectangular coordinates are identified by the **`standard_name`** attribute values **`projection_x_angular_coordinate`** or **`projection_x_coordinate`** (deprecated) and **`projection_y_angular_coordinate`** or **`projection_x_coordinate`** (deprecated), respectively.
...
+
The use of `projection_x_coordinate` and `projection_y_coordinate` was deprecated in version 1.8 of the CF Conventions.
