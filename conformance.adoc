...
* In case (3), the norm ancillary variable must have an auxiliary coordinate variable with a **`gather`** attribute naming a time coordinate variable that has a **`climatology`** attribute. The auxiliary coordinate values must be non-negative integers that are less than the dimension named by the **`gather`** attribute.
