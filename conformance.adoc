...
* The dimensions of each variable named by the **`coordinate`**
  attribute must be a subset of the dimensions named by the
  **`dimensions`** attribute, with two exceptions. First, a label
  variable which will have a trailing dimension for the maximum string
  length. Second a ragged array (Chapter 9, Discrete sampling
...
* Domain variables should not have the **`actual_range`**,
  **`add_offset`**, **`ancillary_variables`**, **`cell_methods`**,
  **`_FillValue`**, **`flag_masks`**, **`flag_meanings`**,
  **`flag_values`**, **`missing_value`**, **`scale_factor`**,
  **`standard_error_multiplier`**, **`standard_name`**, **`units`**,
  **`valid_max`**, **`valid_min`**, or **`valid_range`** attributes.
