...
The __feature__ keywords must include `file`, `format`, `address`, and `shape`.
  ...
  - A data value of a `file` variable must be either an absolute URI or else a relative-path URI reference, after any string substitutions defined by the **`substitutions`** attribute have been applied.
