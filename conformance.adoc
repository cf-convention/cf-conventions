...
* Each of the coordinate variables and scalar coordinate variables identified by the __name__s in the **`anomaly wrt`** entry of **`cell_methods`** (we refer to these variables hereafter as "anomaly coordinate variables") must have a **`standard_name`** attribute.
...
* In case (3), the norm ancillary variable must have an auxiliary coordinate variable of integer type with a **`gather`** attribute naming a time coordinate variable that has a **`climatology`** attribute. The auxiliary coordinate values must be non-negative and less than the dimension named by the **`gather`** attribute.

*Recommendations:*

* In case (2) of the requirements for this section, the norm coordinate variable is recommended not to have any of the Appendix A attributes except for **`cell_methods`** and **`coordinates`**.
