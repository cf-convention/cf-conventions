...
* The tie point variables associated with each
  **`interpolation_variable`** token must all span the same
  dimensions, which comprise a subset of zero or more dimensions of
  the parent data variable with the addition of at least one tie point
  interpolation dimension identified by the **`tie_point_dimensions`**
  attribute. A tie point variable must not span both a tie point
