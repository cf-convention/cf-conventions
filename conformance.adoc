...
* A `location` variable must have a string data type.
  ...
  ** The size of each dimension, except the optional extra trailing dimension, must be the equal to number of valid values in the corresponding row of the `map` variable.
    *** If there is no extra trailing dimension then the variable must have no missing values.
    *** If there is an extra trailing dimension and the variable contains missing values, then those missing values must all be at the end of the extra trailing dimension.
  ...
  ** A data value of a `location` variable must be independent of the order in which string substitutions are applied.

* An `identifier` variable must be either a scalar, or else have the same dimensions in the same order as the `location` variable.
 ** When the `identifier` variable is a scalar it must not contain a missing value.
 ** When the `identifier` variable has the same dimensions as the `location` variable it must have the same distribution of missing values as the `location` variable.

* The `unique_value` variable must have the same number of dimensions as there are aggregated dimensions.
...
* The `map` variable must have an integer data type.
  ...
  *** The sum of the valie values in a row must equal the size of the corresponding aggregated dimension.
