...
* If a boundary variable has an inheritable attribute, i.e. any of those marked "BI" in the "Use" column of http://cfconventions.org/cf-conventions/cf-conventions.html#attribute-appendix[Appendix A], then the same attribute must exist on the parent variable, with the same data type and value.
