...
* An aggregation variable must have an **`aggregated_data`** attribute whose string value comprises blank-separated elements of the form **`feature: variable`**. Each **`variable`** must be the name of a variable in the file.
The **`feature`** keywords must include `file`, `format`, `address`, and `shape`.
