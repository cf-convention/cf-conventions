...
* The tie point variables associated with each
  **`interpolation_variable`** token must all span the same dimensions,
  ...
  data variable with the addition of at least one tie point
...
* When attached to a data variable, the type of the
  **`tie_point_dimensions`** attribute is a string whose value is a list
...
* When attached to a data variable, the type of the
  **`tie_point_indices`** attribute is a string whose value is a list of
