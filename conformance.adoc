...
* A `location` variable must have a string data type.
  ...
  ** The `location` variable must have the same number of dimensions as there are aggregated dimensions.
  ...
  ** A data value of a `location` variable must be either an absolute URI or else a relative-path URI reference.
