...
* The value for seconds in the reference datetime must not equal or exceed 60, except for valid leap seconds if the **`calendar`** is **`utc`**.
* A non-zero time zone offset must not appear in the reference datetime in the **`utc`** and **`tai`** calendars.
* Datetimes before 1972-01-01 0:0:0 are invalid in the **`utc`** calendar, datetimes before 1958-01-01 0:0:0 are invalid in the **`tai`** calendar, datetimes in negative years are invalid in the **`standard`** and **`julian`** calendars, and datetimes in the range from (and including) 1582-10-5 0:0:0 until (but excluding) 1582-10-15 0:0:0 are invalid in the **`standard`** calendar.
Invalid datetimes must not be encoded as time coordinates.
* The reference datetime of a time coordinate variable must be a valid datetime in the specified calendar.

*Recommendations:*

In the first two recommendations, when we refer to a UDUNITS unit of time e.g. **`year`**, we mean the string given or any which is equivalent to it in UDUNITS e.g. **`yr`**, optionally with one of the prefixes of <<table-supported-units>>.
