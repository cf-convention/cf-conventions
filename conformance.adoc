...
* The **`title`** and **`history`** attributes are only defined as of groups (or if groups are not used explicitly, of the root group).
...
* Group attributes must not be used to store metadata normally attached to variables, such us **`_FillValue`**, **`scale_factor`**, **`valid_min`**.
...
* References must be defined by one of the following:
  - Absolute path of the out-of-group item (variable/dimension).
  - Relative path to the out-of-group item (variable/dimension).
  - Item (variable/dimension) name, with no path, if the item can be found in one of the following (ordered by precedence):
    1. In the referring group (thus not an out-of-group reference).
    2. In an ancestor group (starting from the direct ancestor and proceeding toward the root group, until it is found).
    3. In any other group, only if the item is a coordinate variable.
* Paths must follow a UNIX-style file convention.
The delimiting character (group separator) is the slash character ("/").
* Absolute paths must begin with a leading slash "/" to indicate that the location is relative to the root group.
* Relative paths must omit the leading slash "/" to indicate that the location is relative to the referring group.
* Relative paths referencing items out of the referring group and its children shall use the UNIX convention "../" to indicate that the location is higher than the referring group in the group hierarchy.
...
* Out-of-group references with no path are discouraged when the referenced item can't be found in any of the ancestor groups (lateral search strategy).
