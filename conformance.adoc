...
* In a **`cell_methods`** entry of the form __name__**`:`** [__name__**`:`** ...] **`anomaly wrt`** __norm__, the word __norm__ must be the name of a variable that is present in the file, and is named by the **`ancillary_variables`** attribute of the data variable (hence we hereafter refer to it as the "norm ancillary variable").
...
* Each of the coordinate variables and scalar coordinate variables identified by the __name__s in the **`anomaly wrt`** entry of **`cell_methods`** of the data variable (which we refer to hereafter as "anomaly coordinate variables") must have a **`standard_name`** attribute.
* The norm ancillary variable must not have any of the anomaly coordinate variables of the data variable as a coordinate variable or a scalar coordinate variable of its own.
* For each of the anomaly coordinate variables, the norm ancillary variable must have either a coordinate variable or a scalar coordinate variable with the same **`standard_name`** attribute. (We hereafter refer to these variables as "norm coordinate variables.)
* The norm ancillary variable must have a **`cell_methods`** attribute with an entry that refers to all the norm coordinate variables, or more than one entry if they contain **`within`** and **`over`** keywords.
* The dimensions of the norm ancillary variable must satisfy one of the following conditions: (1) The norm ancillary variable has all the same dimensions as the data variable, excluding the dimensions of anomaly coordinate variables, and with the possible inclusion of size-one dimensions for norm coordinate variables; (2) The norm ancillary variable has no dimensions other than size-one dimensions for coordinate variables which have the same **`standard_name`** attributes as anomaly coordinate variables; (3) The norm ancillary variable has a single dimension, which is the dimension of a time coordinate variable that is an ancillary data variable.
