...
Invalid datetimes must not appear as the reference in the **`units`**, and must not be encoded as time coordinates.

*Recommendations:*

In the first two recommendations, when we refer to a UDUNITS unit of time e.g. **`year`**, we mean the string given or any which is equivalent to it in UDUNITS e.g. **`yr`**.
