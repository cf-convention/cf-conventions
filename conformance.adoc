...
* An aggregation variable must have an **`aggregated_data`** attribute whose string value comprises blank-separated elements of the form __feature: variable__.
  ...
  The __feature__ keywords must comprise either all three of the `map`, `location`, and `identifier` keyords; or else both of the `map` and `unique_value` keywords.
  ...
  ** The `identifier` variable must be either a scalar, or else have the same dimensions in the same order as the `location` variable.
  ...
  ** If there are one or more aggregated dimensions then the `map` variable must be two-dimensional.
    ...
    *** The size of the more rapidly-varying dimension (i.e. the second dimension in CDL order) must either be equal the size of the largest of the `unique_value` variable dimensions, or else be equal to size of the largest of the `location` variable dimensions, excluding the extra trailing dimension if the `location` variable has one.

  *** The rows correspond to the aggregated dimensions in the order in which they are defined by the **`aggregated_dimensions`** attribute, and the sum of each row's non-missing values must equal the size of its corresponding aggregated dimension.
