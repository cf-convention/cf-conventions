...
However, a summary with examples can be found in <<mesh-topology-variables>>, and to reduce the chance of ambiguities arising from their accidential re-use, all of the UGRID standardized attributes are specified in <<appendix-mesh-topology-attributes>> and <<attribute-appendix>>.
