...
storing unstructured (or flexible mesh) data in netCDF files
<<UGRID>>. The UGRID conventions description is referenced from,
rather than rewritten into, this document, i.e. the canonical
description of how to store mesh topologies is only to be found at
<<UGRID>>. However, a summary with examples can be found in
