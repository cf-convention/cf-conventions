...
aggregation variable:: A variable whose data is defined by as an aggregation of fragments, rather than containing its own data.
...
Note that an aggregation coordinate variable is stored as a scalar, and must have the same name its aggregated dimension (see <<aggregation-variables>>).
