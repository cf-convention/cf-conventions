...
Alternatively, **`ancillary_variables`** may be used as a status flag indicating operational status of an instrument producing the data or a quality flag indicating the results of a quality control test, or some other quantitative quality assessment, performed against the measurements contained in the source variable.  In these cases, the flag variable will include a standard name that differs from that of the source variable and indicates the specific type of flag the variable contains.  The standard names table includes several types of names that are intended for use in this situation, with a few examples shown below.

.Sample flag variable standard names:
- **`status_flag`** and **`quality_flag`**: general flag categories
- **`climatology_test_quality_flag`**, **`flat_line_test_quality_flag`**, **`gap_test_quality_flag`**, **`spike_test_quality_flag`**: flags indicating the results of common timeseries data quality control tests
- **`aggregate_quality_flag`**: flag indicating an aggregated summary of multiple individual quality flags that use the same flagging scheme

The following example illustrates the use of three of these flags to represent two independent quality control tests and an aggregated flag that combines the results of the two tests (assuming common encoding using the techniques defined in <<flags>>).
...
Further details on implementation of status and quality flag ancillary variables is found in the following <<flags>>.
