...
The biological_taxon_name coordinate included for human readability is mandatory. The biological_taxon_lsid coordinate included for software agent readability is optional, but strongly recommended. If both are present then  biological_taxon_name coordinate must exactly match the name resolved from the biological_taxon_lsid coordinate. If LSIDs are available for some taxa in a dataset then the biological_taxon_lsid co-ordinate should be included and missing data given for those taxa that do not have an identifier.
