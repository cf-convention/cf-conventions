...
| `fv2ll` | Conversion from three-dimensional cartesian vector `(x, y, z)` to geocentric `(latitude, longitude)`| `(lat, lon) = fv2ll(v) = (atan2(v.y, v.x), atan2(v.z, sqrt(v.x * v.x + v.y * v.y))` +
...
Optionally the flag variable `interpolation_subarea_flags`, which must span the interpolation subarea dimension and must include `location_use_3d_cartesian` in the `flag_meanings` attribute. 
...
Optionally the flag variable `interpolation_subarea_flags`, which must span the interpolation subarea dimension 2 and interpolation subarea dimension 1 and must include `location_use_3d_cartesian` in the `flag_meanings` attribute.  

| Coordinate Compression Calculations | 
First calculate the tie point vector representations from the tie point latitude-longitude representations +
`va = fll2v(lla);  vb = fll2v(llb); vc = fll2v(llc); vd = fll2v(lld).` +
Then calculate the three-dimensional cartesian representation of the interpolation coefficients sets from the tie points as well as a point `vp(i2, i1)` between the tie points. If the size of the interpolation subarea in the first dimension `(ib1 - ia1)` is an even number, then the index 
