...
| `fv2ll` | Conversion from three-dimensional cartesian vector `(x, y, z)` to geocentric `(latitude, longitude)`| `(lat, lon) = fv2ll(v) = (fatan2(v.z, fsqrt(v.x * v.x + v.y * v.y)), fatan2(v.y, v.x))` +
...
| `fv2az` | Conversion from three-dimensional cartesian vector `(x, y, z)` to `(azimuth, zenith)` angles | `(azimuth, zenith) = fv2az(v) = (fatan2(y, x), fatan2(fsqrt(x * x + y * y), z))` +

| `fsqrt` | Square Root | `s = fsqrt(t)`

| `fatan2` | Inverse Tangent of y/x | `a = fatan2(y, x)`
