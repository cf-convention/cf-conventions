...
The "Type" values are **S** for string, **I** for integer, and **D** for the type of the variable.
...
| D
| D
| Specifies an variable that defines the boundary condition for each boundary identified by the `boundary_node_connectivity` attribute of the mesh topology variable.
