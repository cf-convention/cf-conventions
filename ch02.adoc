...
Files that follow these conventions must indicate this by setting the NUG defined global attribute **`Conventions`** to the string value "**`CF-1.8`**".
The Conventions version number contained in that string can be used to find the web based versions of this document are from the link:$$http://http://cfconventions.org/latest.html$$[netCDF Conventions web page].
...
Generally it only makes sense to use a single version of the CF Conventions in the `Conventions` attribute.
Software that is able to process files encoded using a given version of the CF Conventions cannot be expected to be able to interpret all aspects of files encoded using a newer version of the standard.
However, later versions of the CF Conventions will not make invalid a compliant usage of earlier versions of the CF terms and forms.
