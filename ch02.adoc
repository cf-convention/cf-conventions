...
Note that this is in conformance with the COARDS conventions, but is more restrictive than the netCDF interface, which allows a wide range of ASCII and UTF-8 characters (link:$$https://docs.unidata.ucar.edu/netcdf-c/current/file_format_specifications.html$$[NUG Appendix B, File Format Specifications]).
The netCDF interface allows leading underscores in names, but the NUG states that this is reserved for system use.
