...
Note that if the aggregation variable indicates that the aggregated data values are packed (as determined by the attributes defined in <<packed-data>>), then the canonical fragment data values will represent packed values in the aggregated data.
In this case, the canonical (i.e. unpacked) fragment data values will be further transformed when the aggregation variable's unpacking is applied.
To avoid the potential for mistakes and confusion as to what the canonical fragment data values represent in the aggregated data, it is recommended that the aggregated variable does not include any packing attributes.
