...
When the data values within a fragment are all the same, for each fragment, the `value` fragment array variable allows the fragments to be represented explicitly by those unique data values, rather than by reference to fragment datasets.
...
If a fragment contains only missing data then this is represented with a missing value in the `value` fragment array variable.
