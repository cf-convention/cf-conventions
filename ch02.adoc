...
Each fragment file name must be a Uniform Resource Identifier (URI) <<URI>>. If the fragment file name is a relative URI (i.e. a URI that does __not__ start with a scheme name followed by a colon, such as `file:`, `http:`, `s3:`, etc.) then it is taken as being relative to the current location of the aggregation file, and if the aggregation file is moved to another location then the fragment file may also need be moved for it to remain accessible.
