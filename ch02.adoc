...
It is recommended that the aggregated data is not packed, because of the potential for mistakes and confusion that could arise from

For instance if a variable has datatype `int` and double precision packing attributes `scale_factor=0.0392156862745098` and add_offset=`275.01960784313724`, then we would expect the upacked data to be in the range `270` to `280`. If that variable is an aggregated variable 
