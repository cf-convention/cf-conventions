...
A fragment contains data with sufficient metadata for it to be correctly interpreted in the context of the aggregation, as described by <<fragment-interpretation>>.
...
Aggregations may also facilitate: data analysis, by avoiding the computational expense of deriving the aggregation at the time of analysis; archive curation, by acting as a metadata-rich archive index; and the post-processing of model simulation outputs, by spanning multiple files written at run time that
...
The data type of the aggregation variable indicates the data type of the aggregated data, and the value of the aggregation variable’s single element can be assigned any value.
...
Any rules imposed by CF that apply to a variable in the CF conventions applies in exactly the same way to an aggregation variable in the same role; and any reference to the dimensions or data of a variable applies to the aggregated dimensions or aggregated data, respectively, of an aggregation variable.
For instance:
...
The presence of an **`aggregated_dimensions`** attribute indicates that the variable it is attached to is an aggregation variable.
...
The aggregated dimensions are partitioned by the fragments, and this partitioning must be consistent across all of the fragments. This means that, for a given aggregated dimension, any two fragments must either span the same part of that dimension, or else or not overlap at all.

The aggregated dimensions are partitioned by the fragments, and this partitioning must be consistent across all of the fragments. This means that, for a given aggregated dimension, any two fragments must either span the same part of that dimension, or else or not overlap at all.

This restriction allows the fragments to be conceptually organised into an orthogonal multidimensionsal __array of fragments__.
Each dimension of the array of fragments corresponds to the aggregated dimension with the same position in the aggregated data.
The size of a dimension of the array of fragments is equal to the number of fragments that span the corresponding aggregated dimension.
The partitioning of the aggregated dimensions into fragments must be consistent across all of the fragments.

The aggregated data are created by concatenating the fragments' data (in their canonical forms, see <<fragment-interpretation>>) along each dimension of the __array of fragments__.
...
[cols="a,a"]
|===============
| *Array of fragments position `[0, 0, 0]`*

Fragment location: `file_A.nc` +
Fragment data shape: `(17, 91, 180)` +
`17` vertical levels  +
`[90, 0]` degrees north  +
`[0, 180)` degrees east | **Array of fragments position `[0, 0, 1]`*
...
| *Array of fragments position `[0, 1, 0]`*

Fragment location: `file_C.nc` +
Fragment data shape: `(17, 45, 180)` +
`17` vertical levels +
`(0, -45]` degrees north +
`[0, 180)` degrees east | *Array of fragments position `[0, 1, 1]`*
...
| *Array of fragments position `[0, 2, 0]`*

Fragment location: `file_E.nc` +
Fragment data shape: `(17, 45, 180)` +
`17` vertical levels +
`(-45, -90]` degrees north +
`[0, 180)` degrees east | *Array of fragments position `[0, 2, 1]`*
...
This attribute takes a string value comprising blank-separated elements of the form "__feature: variable__", where __feature__ is a case-sensitive keyword that identifies a feature of the array of fragments, and __variable__ is a variable in the aggregation file which provides values for that feature.
The features unambiguously define the array of fragments.
The order of elements in the **`aggregated_data`** attribute is not significant.

The features must comprise either all three of the `map`, `location`, and `identifier` keywords; or else both of the `map` and `unique_value` keywords.
No other combinations of keywords are allowed.
...
The `map` variable is two-dimensional: the rows (i.e. the slowest-varying dimension, and the first dimension in CDL order) correspond to the aggregated dimensions in the same order; and the columns correspond to the number of fragments along each of the aggregated dimensions.
The `map` variable data provides the sizes of the fragments along each of the aggregated dimensions, in the same relative ordering that the fragments occur in the array of fragments.
Since the aggregated dimensions may be spanned by differing numbers of fragments, the rows of the `map` variable are padded with missing values to create a rectangular array.
The part of each aggregated dimension that is occupied by a fragment is defined by the fragment's size along that dimension, offset by the sum of the fragment sizes that precede it.

For instance, in the <<example-array-of-fragments>> example, the corresponding `map` variable has 3 rows (one for each of the aggregated dimensions), and 3 columns (to allow space for the largest number of fragments along any of the aggregated dimensions).
Each of these rows contains the sizes of the fragments along that dimension, padded with missing values (denoted by `_`) to create a rectangular array, as follows:
...
180, 180,  _
```
The fragment in position `[0, 1, 1]` of this array of fragments therefore occupies elements 0 to 16 (i.e. the entirety) of the Z aggregated dimension, 91 to 135 of the Y aggregated dimension, and 180 to 359 of the X aggregated dimension (using zero-based indices).
...
In general, its dimensions correspond to, and have the same sizes as, the dimensions of the array of fragments; and its data provide a location for each fragment.
...
The `identifier` variable, that may have any data type, defines how to find extract each fragment from within its fragment dataset, i.e. the identifier of the fragment.
In general, the dimensions of the `identifier` variable correspond to those of the `location` variable, and its data contain an identifier corresponding to each fragment location.
If the identifiers are the same for all fragments then the `identifier` variable may, however, be a scalar whose single data value is the identifier common to all fragments.
The identifier for a netCDF fragment dataset must be the string-valued variable name of the fragment.
See <<example-L.1>> and <<example-L.5>>.

===== unique_value

When the data values within a fragment are all identical, for each fragment, the `unique_value` variable allows all fragments to be defined solely by these unique values explicitly in the aggregation file, rather than by reference to external fragment datasets.
The `unique_value` variable dimensions correspond to, and have the same sizes as, the dimensions of the array of fragments; and its data contain each fragment's unique value.
...
See <<example-L.6>>, which uses an aggregation ancillary variable to make global attributes from the fragment datasets available in the aggregation file.
...
Note that the row sums of the `fragment_map` variable are `17`, `181`, and `360`, which equal the sizes of the `level`, `latitude`, and `longitude` aggregated dimensions, respectively.
...
All of the fragments are combined to form the aggregated data such that each aggregated data value comes from exactly one fragment, and each fragment data value provides exactly one aggregated data value.
Because fragments may be encoded in many different but equivalent ways, we define a __canonical form__ of a fragment's data that provides a view of the fragment for which all data values are consistent with those from other fragments, as well as with the attributes of the aggregation variable.
When constructing the aggregated data, it is assumed that each fragment's data will have been transformed to its canonical form.
The canonical form of a fragment's data is such that:

1. The fragment's data has the same number of dimensions as the aggregated data.

2. The fragment's data are unpacked (as described in <<packed-data>>).

3. The fragment's data have the same units as the aggregation variable (e.g. as required when aggregating time fragments whose units have different reference date/times).

4. The fragment's data have the same data type as the aggregation variable.

5. Missing values in the fragment's data the same as those defined by the aggregation variable.
...
* A fragment may have fewer (but never more) dimensions than the aggregated data, and any missing dimensions must have size 1. (e.g. as could be the case when aggregating two-dimensional fragments into three-dimensional aggregated data).

* If the aggregation variable indicates that the aggregated data values are packed (as described in <<packed-data>>), then the canonical form of the fragment data values will be subject to the
aggregation variable's unpacking.

* It is the responsibility of the creator of the aggregation file to ensure that all valid fragment data values are different from any of the missing values defined by the aggregation variable.