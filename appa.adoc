...
parameterized dimensional vertical coordinate values computed according to the
formula in the definition.
FIXME: "parameterized dimensional vertical coordinate" ... obsolete terminology?
...
| Identifies variables which are named by attributes in the file but which are not present in the file.
