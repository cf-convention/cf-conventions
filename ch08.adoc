...
same number of tie points, and therefore have different numbers of
...
variables corresponding to a particular interpolation dimension
spanning must be separately associated with the corresponding
interpolation variable, even if that interpolation variable is the
same in each case. For instance, if tie point variables `lat` and
`lon` span dimension `tp_dimension1` and `time` spans dimension
`tp_dimension2`, and all three are to interpolated according to
interpolation variable `linear`, then the **`tie_points`** attribute
could be `lat: lon: linear time: linear`.
