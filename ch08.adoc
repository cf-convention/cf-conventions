...
If a tie point coordinate variable has cell boundaries bounds then it must have the attribute **`bounds`** that names the variable that contains the vertices of the cell boundaries. The bounds should be the same as the bounds of the corresponding target grid cells, unless there is a non-zero offset, in which case the offset applies to the bounds values in exactly the same manner as it does to the cell corodinates. It is thereforefore likely that tie point cells will be non-contiguous.
