...
increasing. An interpolation subarea must span
at least two points in each of its corresponding interpolated
dimensions, therefore the tie point indices that 
define an
interpolation subarea must all be different. When two adjacent values differ 
by one, it indicates the location (in index space) of
...
variables:	       
  ...
  char linear ;			
    ...
    linear:tie_point_mapping = "y: y_indices tp_y  x: x_indices tp_x"  ;
