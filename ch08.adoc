...
and interpolation configuration attributes. The projection coordinates
are two-dimensional, but are only linearly interpolated in one of
their dimensions - the one which is given by the
**`tie_point_indices`** attribute.
