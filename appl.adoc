...
data:
  ...
  latitude = ... ;
  longitude = ... ;
  fragment_location = "${local}January-March.nc",
                      _,
                      "${local}April-December.nc",
                      "${remote}April-December.nc" ;
  fragment_location_time = "${local}January-March.nc",
                           _,
                           "${local}April-December.nc",
                           "${remote}April-December.nc" ;
  fragment_address = "temperature" ;
...
The distribution of missing values in the `fragment_shape` variable indicates that the `level` aggregated dimension is spanned by 1 fragment, the `latitude` aggregated dimension is spanned by 3 fragments, and the `longitude` aggregated dimension is spanned by 2 fragments; and that the shape of the implied fragment array is `(1, 3, 2)`.
...
variables:
  ...
  string fragment_id(f_time, f_level, f_latitude, f_longitude) ;
    fragment_id:long_name = "Fragment dataset unique identifiers" ;
...
----
This example is similar to <<example-L.1>>, but now the **`aggregated_data`** attribute also includes the non-standardized feature keyword `id`, which has the corresponding variable `fragment_id`.
