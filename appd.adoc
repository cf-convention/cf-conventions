...
where `zeta_S = ln(p0)`, `zeta(k) = zeta_S + ln(eta(k))` and `zeta_U = min(zeta(k))`. `rmin` and `rmax` can be adjusted to control the rectification of levels over high terrain as described by Girard et al. 2014 (https://doi.org/10.1175/MWR-D-13-00255.1).
...
To indicate the `standard_name` of this quantity, the coordinate variable may be given the attribute `computed_standard_name="air_pressure"`. 
