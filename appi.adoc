...
domain axis constructs of the coordinate construct, with the addition
of an extra ragged dimension that indexes the parts for each cell. For
...
explicitly describes the geometry type, identifies the node coordinate
variables that contain the cell vertices, and references the node
count variable that contains the number of parts per cell. When a
geometry container variable is not present then the bounds contain
exactly one part and their geometry type is implied by convention: for
multidimensional auxiliary coordinates each cell is a single polygon,
and for all other types of coordinate each cell is a single line
