...
image::images/mesh_figure.svg["A figure of a triangular cell next to a quadrilateral cell, sharing one boundary. the quad cell has two nodes that are shared with the triangle, and two other nodes defining the quad. The triangle is defined by two of the nodes of the qad and one additional node.", 25%, pdfwidth=25vw, align="center"]
