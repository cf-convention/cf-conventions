...
cells that span a collection of dimensions, with cell coordinates,
cell measures, and coordinate reference systems.
...
is needed by a data variable to describe its domain, then that
attribute would also be needed by the equivalent domain variable.
...
dimensions that may be be spanned by variables referenced from any of
...
given by the `coordinates` attribute of a domain variable must span a
subset of zero or more of the dimenions given by the `dimensions`
attribute.
