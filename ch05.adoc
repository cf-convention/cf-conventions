...
variable__ defines a mesh topology for one or more domains, i.e. the
topological connections within each domain are defined, as well as the
topological connections between the domains. A data variable may use
one of a mesh topology variable's domains by identifying the mesh
topology variable alongside the required constituent domain, typically
