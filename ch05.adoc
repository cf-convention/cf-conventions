...
edges, faces or volumes of the mesh topology (see example
<<example-mesh-topology-variable>>).

A __location index set variable__ defines a subset of locations of a
...
The canonical definitions of mesh topology variables and location
index set variables are given by the UGRID conventions <<UGRID>>, but
their standardized attributes, many of which are optional, are listed
in <<appendix-mesh-topology-attributes>> and <<attribute-appendix>>.
