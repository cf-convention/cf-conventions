...
It is recommended that a domain variable does not have any other
attributes that are also used to directly describe data values,
defined in <<attribute-appendix>> as those attributes that are used
for non-coordinate data which also do not have domain variable nor
global use.
