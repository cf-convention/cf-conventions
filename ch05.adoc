...
attribute is a blank separated list of the dimensions names. There is
...
string. This could be the case, for instance, if the dimensions of the
domain are all defined implicitly by scalar coordinate variables.
...
Multiple domain variables may exist in a file with, or without other
data variables. Note that the data variable attributes describing its
...
<<example-h.7>> have been replaced by the domain variable
**`domain`**.
