...
A domain describes cell locations and properties. It defines cells
that span a collection of dimensions by identifying the cell
coordinates, measures, and coordinate reference systems.

A domain variable provides the description of the domain, in the
absence of any data values, via a collection of attached
attributes. It is of arbitrary type since it contains no data. Its
purpose is to act as a container for the attributes that define the
domain. A domain variable can not be utilized by a data variable, as
that defines its own domain via its own attributes. The purpose of the
domain variable is to provide domain information to applications that
have no need of data values at the domain's locations, thus removing
any ambiguity when retrieving a domain from a dataset. A domain
variable may exist in a file with or without other data variables.

The domain variable supports the same attributes as are allowed on a
data variable for describing aspects of the domain,
i.e. `cell_measures`, `coordinates`, `geometry`, and `grid_mapping`,
with exactly the same meanings and syntaxes as descibed in
<<attribute-appendix>>.
...
required dimensions (as could be the case for a discrete axis). The
value of the `dimensions` attribute is a blank separated list of the
dimensions names. There is no restriction on the order in which the
dimension appear in the `dimensions` attribute string. The presence of
a `dimensions` attribute will identify the variable as a domain
variable.
...
shares its name with a dimension given by the `dimensions` attribute
will considered as part of the domain definition.
...
In this example, the data variable `xwind` from
<<independent-coordinate-variables-ex>> has been replaced by the
...
variables:
  ...
  double time
    ...
    atime:units = "days since 2000-12-01 00:00" ;
  float rlon(rlon) ;
    rlon:long_name = "longitude in rotated pole grid" ;
    rlon:units = "degrees" ;
    rlon:standard_name = "grid_longitude";
...
[[a-domain-containing-cell-areas-for-a-sphericalgeodesic-grid]]
...
In this example, the data variable `PS` from
<<cell-areas-for-a-spherical-geodesic-grid>> has been replaced by the
domain variable `domain`.
...
In this example, the data variable `someData` from
<<timeseries-with-geometry>> has been replaced by the domain variable
`domain`.
...
<<example-h.7>> have been replaced by the domain variable `domain`.
