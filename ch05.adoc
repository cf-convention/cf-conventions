...
coordinates, cell measures, and coordinate reference systems.

A data variable defines its own domain via its own attributes, but a
domain variable provides the description of a domain in the absence of
any data values, via a collection of attached attributes. It is of
arbitrary type since it contains no data. Its purpose is to act as a
container for the attributes that define the domain. A domain variable
can not be utilized by a data variable, rather its purpose is to
provide domain information to applications that have no need of data
values at the domain's locations, thus removing any ambiguity when
retrieving a domain from a dataset. A domain variable may exist in a
file with or without other data variables.
