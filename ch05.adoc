...
[[healpix-with-compression-by-gathering]]
...
dimensions:
    time = 100;
    // TODO: the cell dimension won't be available when encoding in zarr (as no variable uses that dimension)
    // TODO: this might or might not be a problem (the size is always 12*4**crs:refinement_level)
    cell = 49152;
    cell_id = 15234;
  variables:
    float time(time) ;
      time.units = "days since 2024-09-04" ;
    float temp(time, cell_id) ;
      temp:standard_name = "air_temperature" ;
      temp:units = "K" ;
      temp:grid_mapping = "crs: cell" ;
    float pres(time, cell_id) ;
      pres:standard_name = "air_pressure" ;
      pres:units = "Pa" ;
      temp:grid_mapping = "crs: cell" ;
    int cell_id(cell_id) ;
      cell_id:compress = "cell" ;
    int crs ;
