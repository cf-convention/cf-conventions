...
that span a collection of dimensions with cell coordinates, cell
...
data variable for describing the domain, i.e. `cell_measures`,
`coordinates`, `geometry`, and `grid_mapping`, with exactly the same
meanings and syntaxes as described in <<attribute-appendix>>. If an
attribute is required by a data variable to describe its domain, then
that attribute must also be present on the equivalent domain variable.
...
will be considered as part of the domain definition.

Any other domain variable attributes are allowed, but have no
