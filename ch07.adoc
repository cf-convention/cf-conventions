...
It can be used only for anomalies with respect to climatology (temporal anomalies), and the information it provides about the definition of the norm is insufficient for some use-cases.

In the remainder of this section and in the following two (<<anomalies-norm-data>> and <<anomalies-norm-metadata>>) we describe a convention for anomalies with respect to a statistical norm that can be calculated from any single dimension or combination of dimensions, including the commonest case of temporal anomalies (i.e. with respect to a climatological norm).
...
The convention of this section provides more information than the convention of <<temporal-anomalies>> about the climatological norm in the case of temporal anomalies.
...
They are not coordinate variables of  __N__ or __P__, but __N__ and __P__ have coordinate variables for the same quantities as the anomaly coordinate variables.
