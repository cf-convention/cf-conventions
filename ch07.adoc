...
Although it is formally a data variable, CF recommends that the norm metadata variable not have any of the attributes of <<attribute-appendix>> other than **`cell_methods`**, **`coordinates`** and **`_FillValue`**.
...
In this case, the norm metadata variable can have more than one element, and its values are immaterial.
...
variables:
  float delta_tas(time,latitude,longitude); // anomaly data variable
    ...
    delta_tas:coordinates="climatological_time";
    delta_tas:ancillary_variables="climatological_tas";
  float climatological_tas; // norm metadata variable
    climatological_tas:coordinates="climatological_time";
    climatological_tas:cell_methods="climatological_time: mean";
  double time(time); // anomaly coordinate variable
    time:standard_name="time";
...
The mapping between the anomaly time axis of the anomaly variable and the climatological time axis of the norm is recorded by an auxiliary coordinate variable pf integer type named by the **`coordinates`** attribute of the norm metadata variable.
