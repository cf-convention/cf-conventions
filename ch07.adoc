...
If a **`coordinates`** attribute is carried by the geometry container variable or its parent data variable, then those coordinate variables which have a meaningful correspondence with node coordinates may have a **`nodes`** attribute that names the corresponding node coordinates.
