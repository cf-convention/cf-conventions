...
The dimensions of the measure variable should be the same as or a subset of the dimensions of the variable to which they are related, but their order is not restricted.
...
In the specification of this attribute, __name__ can be a dimension or scalar coordinate of the variable, a valid standard name, or the word "**`area`**".
(See <<cell-methods-no-coordinates>> concerning the use of standard names in cell_methods.)
The values of __method__ should be selected from the list in <<appendix-cell-methods>>, which includes `point`, `sum`, `mean`, and `variance`, among others.
...
Various calculational procedures might be consistent with a specified cell_methods.  As a default, a method requiring a sum over samples 
(e.g., a “mean” or “variance”) should (except as noted below) be weighted by the size of the interval or the size of the domain 
represented by the sample. Thus, a time mean computed from multiple samples is assumed to be weighted by the length of the time interval 
associated with each sample. For example, an annual mean could be computed from monthly means, but each month would weighted by the number 
of days in the month.  The exception to this default treatment is that for a vertical coordinate, the weights should be proportional to 
the mass of the medium per unit area.  For a pressure coordinate, this is just the pressure spanned by the layer, but for a height coordinate, 
the thickness would need to be weighted by density.  If the weighting applied is different from the default defined here, this can be 
indicated by providing supplemental information in the **`cell_methods`** (see  <<recording-spacing-original-data>>).
...
If the same method applies to multiple axes, then that method can be prefixed by the names of all the dimensions involved (listed in any order, since in this case the order must be immaterial).
The result of grouping axes together is usually different from sequentially applying the method over one axis and then another. 
For instance, the standard deviation of topographic height within a longitude-latitude gridbox is indicated by **`cell_methods="lat: lon: standard_deviation"`** or, equivalently, as provided for in 
the following paragraph, **`cell_methods="area: standard_deviation"`**.)
...
The horizontal coordinate variables to which **`area`** refers are in this case not explicitly indicated in **`cell_methods`** but can 
be identified, if necessary, from attributes attached to the coordinate variables, scalar coordinate variables, or auxiliary 
coordinate variables, as described in <<coordinate-types>>.  When **`area`** is used to reference the horizontal coordinates, then 
statistics involving sums of 
samples should be computed using weights proportional to area or, in the case of a method also involving the vertical coordinate (e.g., 
**`area: depth: mean`**) weights proportional to mass.  Such weighting is the usual one applied in calculating a mean over area or over mass, 
and thus there is an advantage to using **`area`** rather than, say, **`lon: lat:`** in **`cell_methods`**.  (Specifying **`lon: lat: mean`** for spatial 
means computed from samples obtained from a latitude-longitude grid would be interpreted as being weighted by the product of the grid-cell 
longitude and latitude angular widths, rather than the area of the cells.) Any alternative to the default interpretations regarding 
weighting can be indicated by providing supplemental information in the **`cell_methods`** (see  Section 7.3.3. “Recording the spacing of 
the original data and other information”).


[[statistics-applying-portions]]
==== Statistics applying to portions of cells

By default, the statistical method indicated by **`cell_methods`** is assumed to have been evaluated over the entire spatio-temporal extent of the cell.
Sometimes, however, it is useful to limit consideration to only a portion of a cell or subset of cells (e.g. a mean surface temperature over the sea-ice portion of a cell or the surface snow depth, but only considering times when sea ice exists in the cell). 
...
Here __name__ could, for example, be **`area`** and __type__ may be any of the strings permitted for a variable with a **`standard_name`** 
of **`area_type`**. Note that when a time dimension is named, the “where” in the **`cell_methods`** would be interpreted as meaning “when”, 
rather than “where”;  there are other cases for which “where and when” would be the appropriate interpretation.   
As an example, for **`cell_methods=“area: mean where sea_ice”`**, the data would represent a mean over only the sea ice portion of the 
grid cell.  As another example, consider surface temperature with a **`cell_methods=“time: mean where sea_ice”`**.  In this case there would 
be no contribution to the mean from ice-free cells.  One limitation on application of this first convention for identifying the cell 
portion of interest should be noted:
if the data writer expects __type__ to be interpreted as one of the standard **`area_type`** strings, then none of the variables in the 
netCDF file should be given a name identical to that of the string (because the second convention, described in the next paragraph, 
takes precedence).
...
Here, _type1_ can be any of the possibilities allowed for _typevar_ or _type_ (as specified in the two paragraphs of the last Example above).
...
A `cell_methods` attribute with a string of the form "mean where _type1_ over _type2_" indicates the mean is calculated by integrating over the _type1_ portion of the cell and dividing by the area of the _type2_ portion.
In particular, a `cell_methods` string of the form "mean where _type1_ over all_area_types" indicates the mean is calculated by integrating over the _type1_ portion of the cell and dividing by the entire area of the cell.
(Note that `all_area_types` is one of the valid strings permitted for a variable with the `standard_name` `area_type`.)
If "over _type2_" is omitted, the mean is calculated by integrating over the _type1_ portion of the cell and dividing by the area of this portion.  Note that when computing the mean from grid-cell values within a region, the integrals referred to above will in practice be sums with samples, by default, weighted by the cell areas.
...
[[recording-spacing-original-data, Section 7.3.3, "Recording the spacing of the original data and other information"]]
...
For instance, the time mean of a variable quantifying the “age of surface snow” might be computed with each sample weighted by the mass of surface snow rather than the default (which, for regularly spaced samples, would be to equally weight them). This alternative to the default could be indicated as **`time: mean (weighted by mass of surface snow)`** or **`time: mean (interval: 1 day comment: weighted by mass of surface snow)`**.
