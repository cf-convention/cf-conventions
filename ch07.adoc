...
A data variable __A__ containing anomalies with respect to a norm is notionally the difference between a data variable __P__ containing the original data and a data variable __N__ containing the statistical norm of the quantity, although __P__ is usually not present in the dataset, and __N__ is often absent too.
...
The commonest kind of anomaly __A__ is the difference between the value __P__ of a quantity and the mean __N__ of the same quantity over some range of time coordinates, usually called the "climatological normal", the "climate normal", or the "climatology".
...
In the case of an anomaly __A__ with respect to a climatology, __N__ has all the same dimensions and coordinate variables as __A__ except for time.
__N__ is computed from the temporal variation of __P__.

CF offers two conventions for describing anomaly data.
The convention of <<temporal-anomalies>> depends on special standard names and can be used only for anomalies with respect to climatology (temporal anomalies).
It is simple but the information it provides about the definition of the norm is insufficient for some use-cases.
The conventions introduced in <<anomalies-norm-intro>> can be used for anomalies with respect to any dimension and provide more information about the norm, but are consequently more complicated.
...
This variable may be either a scalar coordinate variable, as in Example 7.15, or a coordinate variable with a single size-one dimension.
...
In Example 7.16, we illustrate the ambiguity in this respect of Example 7.15.
...
----
with dimension **`climonth=1`** for the climatological month of July.
...
This convention also allows us to clarify the definition of the climatological norm in the case of temporal anomalies (removing the ambiguity of Example 7.16).
...
If the data variable containing the norm __N__ is present in the dataset, __norm__ can be the name of that variable, e.g. a data variable containing zonal means for anomalies with respect to the zonal mean.
...
The anomaly data variable must name the norm data variable in its **`ancillary_variables`** attribute (<<ancillary-data>>), in order to indicate the link between them.
...
Whereas the anomaly coordinate variable(s) are usually multivalued, the norm coordinate variable(s) must be of size one.
This is because the norm is a statistic that represents the variation of __P__ within the range(s) of the norm coordinate variable(s).
...
Example 7.17 shows how the **`cell_methods`** of the norm data variable clarifies the ambiguity in the convention of Example 7.15 illustrated in Example 7.16.
...
The anomaly data variable must name the norm metadata variable in its **`ancillary_variables`** attribute (<<ancillary-data>>), in order to indicate the link between them.
...
These are exactly the same norm coordinate variables (usually there is only one of them) as in <<anomalies-norm-data>>, and they must have the same sizes and bounds as in that case.
The norm metadata variable must have a **`cell_methods`** attribute with a _single_ entry, which refers to the norm coordinate variable, or the combination of them if more than one, to indicate how __N__ was computed from the variation of __P__, as in <<anomalies-norm-data>>.
...
The norm metadata variable has only one element.
...
If the sole, or every, norm coordinate variable is a scalar coordinate variable, the norm metadata variable will have no dimensions i.e. it will be a scalar.

Although it is formally a data variable, it is recommended that a norm metadata variable does not have any of the attributes of Appendix A except for **`cell_methods`**, and **`coordinates`** if required.
It represents a norm data variable which must contain the same quantity as the anomaly data variable (standard name, units, etc.); hence it does not need these attributes.

The use of norm metadata variables is illustrated by Examples 7.18, 7.19 and 7.20.

[[anomalies-wrt-climatology-ex]]
[caption="Example 7.18. "]
.Temporal anomalies with a climatological norm metadata variable
====
This example shows how the metadata of Example 7.17 can be recorded using a norm metadata variable.
...
----
As in Example 7.17, the metadata of **`delta_tas`** is the same in the two cases.
They are distinguished by the **`cell_methods`** and time coordinates of **`climatological_tas`**.
====
...
As shown in Example 7.21, if the data variable has a discrete sampling geometry, the number of geometries is the length of the instance dimension (Section 9.2).
