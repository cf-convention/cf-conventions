...
Nonetheless, the bounds may still be present, for instance if the grid is shared by other data variables that pertain to cells, and it may be useful for generic applications to be given some indication of cells for graphical purposes.
...
----
The boundary variable **`time_bnds`** associates an time point **`i`** with the time interval whose boundaries are **`time_bnds(i,0)`** and **`time_bnds(i,1)`**.
The instant **`time(i)`** should be contained within the interval, or be at one end of it.
For instance, for some **`i`** we might have **`time(i)=10.5`**, **`time_bnds(i,0)=10.0`**, **`time_bnds(i,1)=11.0`**.
Since the coordinate variables are increasing e.g. **`time(1)`** &gt; **`time(0)`**, so must the bounds i.e. **`timebnd(i,1)`** &gt;= **`timebnd(i,0)`** for all **`i`** (<<img-bnd_1d_coords>>).
If adjacent intervals are contiguous, the shared endpoint must be identical.
For example, if the intervals that contain grid points **`time(i)`** and **`time(i+1)`** are contiguous, it is required that **`timebnd(i+1,0)`** = **`timebnd(i,1)`**.
====
...
[.text-center]
.Order of **`lonbnd(i,0)`** and **`lonbnd(i,1)`** as well as of **`latbnd(i,0)`** and **`latbnd(i,1)`** in the case of one-dimensional horizontal coordinate axes. Tuples **`(lon(i),lat(j))`** locate the gridpoints. The four grid cell vertices are given by **`(lonbnd(i,0),latbnd(j,0))`**, **`(lonbnd(i,1),latbnd(j,0))`**, **`(lonbnd(i,1),latbnd(j,1))`** and **`(lonbnd(i,0),latbnd(j,1))`**.
...
[caption="Figure {doc-part}.{counter:figure}. ", reftext=Figure {doc-part}.{figure}]
[.text-center]
.Order of **`lonbnd(j,i,0)`** to **`lonbnd(j,i,3)`** and of **`latbnd(j,i,0)`** and **`latbnd(j,i,3)`** in the case of two-dimensional horizontal coordinate axes. Tuples **`(lon(j,i),lat(j,i))`** locate the gridpoints and tuples **`(lonbnd(j,i,n),latbnd(j,i,n))`** locate the grid cell vertices.
