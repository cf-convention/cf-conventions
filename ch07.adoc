...
__name__ can be a dimension of the variable, a scalar 
                        ...
                        (See <<cell-methods-no-coordinates>> concerning the use
                        of standard names in cell_methods.) The values of __method__ should be selected from the list in <<appendix-cell-methods>> , which includes `point` , `sum` , `mean` , `maximum` , `minimum` , `mid_range` , `standard_deviation` , `variance` , `mode` , and `median` . Case is not significant in the method name. Some methods (e.g., `variance` ) imply a change of units of the variable, as is indicated in <<appendix-cell-methods>> .
