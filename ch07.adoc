...
__N__ is computed from the temporal variation of __P__, and is most often either a time-mean over a continuous period of several entire years or a climatological mean (<<climatological-statistics>>).
...
In the remainder of this section and in the following two (<<anomalies-norm-data>> and <<anomalies-norm-metadata>>) we describe a convention for anomalies with respect to a statistical norm that can be calculated from any single dimension or combination of dimensions.
...
The convention of this section can be used for the commonest case of temporal anomalies, and in that case it provides more information than the convention of <<temporal-anomalies>> about the norm.
...
Therefore we call these the "anomaly coordinate variables".
...
The anomaly coordinate variables are not coordinate variables of  __N__, but  __N__ has a coordinate variable or scalar coordinate variable for the same physical quantity as each of the anomaly coordinate variables.
We call these corresponding variables the "norm coordinate variables".
...
Corresponding to each of the anomaly coordinate variables (usually there is only one), the __norm__ variable must have either a norm coordinate variable (with its dimension included among the dimensions of the norm data variable) or a scalar coordinate variable (named in the **`coordinates`** attribute of the norm data variable) for the same physical quantity as the anomaly coordinate variable.
...
Any norm coordinate variable dimensions (which necessarily have size one) must be included among the dimensions of the anomaly data variable, and any scalar norm coordinate variables must be named in the **`coordinates`** attribute of the anomaly coordinate variable.
A norm coordinate variable cannot have a dimension greater than 1; for multivalued climatological norm coordinate variables, a norm metadata variable is required (<<anomalies-norm-metadata>>).
...
Example 7.15 shows how the **`cell_methods`** of the norm data variable clarifies the how it was calculated.
By contrast, metadata following the convention of <<temporal-anomalies>> is incomplete, as illustrated in Examples 7.19 and 7.20.
...
variables:
  ...
  float climatological_tas(climatological_time,latitude,longitude);
    ...
    climatological_tas:units="degC";
    climatological_tas:units_metadata="temperature: on_scale";
    climatological_tas:cell_methods="climatological_time: mean within years climatological_time: mean over years";
  double time(time);
    time:standard_name="time";
    time:units="days since 2023-7-16";
    time:bounds="time_bounds";
...
In this case, the word __norm__ in **`cell_methods`** is the the name of a variable in the dataset that is __not__ the norm data variable (although this may also be present in the dataset).
The purpose of this norm metadata variable is to host the coordinate variables and the **`cell_methods`** which define the norm.
...
For each of the anomaly coordinate variables (usually there is only one), the __norm__ metadata variable must have a scalar coordinate variable (named in the **`coordinates`** attribute of the both the norm metadata variable and the anomaly data variable) for the same physical quantity as the anomaly coordinate variable, except if the anomalies are with respect to multivalued climatological time.
The norm coordinate variable(s) must have boundary variables to indicate the coordinate ranges over which __N__ was calculated from __P__.
...
This situation is described in detail after Example 7.18.

The norm metadata variable must have a **`cell_methods`** attribute (<<cell-methods>>) with a _single_ entry, which refers to the dimension of the norm coordinate variable, or the combination of them if more than one, or to the multivalued climatological time dimension (see below), to indicate how __N__ was computed from the variation of __P__, as in <<anomalies-norm-data>>.

The norm metadata variable must have no scalar coordinate variables other than the norm coordinate variables.
Except in the case of multivalued climatological time (see below), the norm metadata variable must be a scalar, whose value is immaterial and of arbitrary type.

Although it is formally a data variable, it is recommended that a norm metadata variable does not have any of the attributes of Appendix A except for **`cell_methods`** and **`coordinates`**, which are both mandatory.
...
Multivalued climatological time is discussed and illustrated after Example 7.18.
...
For example, suppose that __A__ contains anomalies for the months of June, July and August of 2024 and 2025 with respect to the 30-year climatological means of those months for 1991--2020.
...
In this situation, the dimension of the anomaly coordinate variable of time (with size 6 in this example) is also a dimension of the norm metadata variable.
The norm metadata variable has an auxiliary coordinate variable for this dimension, containing a list of indices to the climatological time coordinate variable of __N__.
...
The list variable is the index of the element of the climatological time variable of __N__ that corresponds to each element of the anomaly time coordinate variable of __A__.
