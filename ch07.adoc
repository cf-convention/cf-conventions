...
It has an auxiliary coordinate variable, with the same dimension as itself, named by its **`coordinates`** attribute, but it has no scalar coordinate variables (unlike in all other cases, which have norm scalar coordinate variables).
