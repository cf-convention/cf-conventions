...
__N__ is usually either a mean over a number of entire years or a climatological mean (<<climatological-statistics>>).
...
The convention of <<temporal-anomalies>> epends on special standard names and can be used only for anomalies with respect to climatology (temporal anomalies).
It is simple but the information it provides about the definition of the norm is insufficient for some use-cases.
The convention of <<general-anomalies>> can be used for anomalies with respect to any dimension and provides more information about the norm, but is consequently more complicated.
...
This variable may be either a scalar coordinate variable, as in <<anomaly-with-reference-epoch-ex>>, or a coordinate variable with a single size-one dimension.
...
----
The data variable **`delta_tas`** contains daily maximum temperatures for 16th-19th July 2023 expressed as anomalies with respect to a climatological normal.
...
The climatological reference period is defined as 1990-2019 by the bounds of **`reference_epoch`**.
...
In <<reference-epoch-ambiguity-ex>>, we illustrate the ambiguity of <<anomaly-with-reference-epoch-ex>> in this respect.
...
These two possibilities, and others, cannot be distinguished with this convention, but they can be distinguished by the conventions introduced in <<anomalies-norm-intro>>.


[[anomalies-norm-intro, Section 7.5.2, "Anomalies with respect to a norm variable"]]
==== Anomalies with respect to a norm variable

The convention of this and the two following subsections can be used to describe anomalies with respect to a statistical norm calculated from any single dimension or combination of dimensions, including the commonest case of temporal anomalies (i.e. with respect to a climatological norm, as in <<temporal-anomalies>>).
...
This convention also allows us to clarify the definition of the climatological norm in the case of temporal anomalies (removing the ambiguity of <<reference-epoch-ambiguity-ex>>).

A data variable is described as an anomaly using this convention by including in its **`cell_methods`** attribute a string of the form "__name__**`:`** [__name__**`:`** ...] **`anomaly wrt`** __norm__".
...
Usually there is only one __name__ and one anomaly coordinate variable, and usually it is a spatiotemporal coordinate variable.

For a data variable containing anomalies with respect to climatology, __name__ identifies the time coordinate or scalar coordinate variable as the anomaly coordinate e.g. "**`time: anomaly wrt`** __norm__".
...
There are two alternatives for __norm__, described in <<anomalies-norm-data>> and <<anomalies-norm-metadata>>.


[[anomalies-norm-data, Section 7.5.3, "Anomalies with respect to a norm data variable"]]
...
If the anomaly data variable has a **`standard_name`** attribute, it must __not__ be a standard name ending in **`_anomaly`**, and the __norm__ variable must have the same attribute.
...
We call these the __norm coordinate variables__ (usually there is only one).
...
A norm data variable for a climatological statistic (in the sense of <<climatological-statistics>>) has a norm coordinate variable which
...
<<reference-epoch-distinguishing-ex>> shows how the **`cell_methods`** of the norm data variable clarifies the ambiguity in the convention of <<temporal-anomalies>> illustrated in <<reference-epoch-ambiguity-ex>>.

[[reference-epoch-distinguishing-ex]]
[caption="Example 7.17. "]
.Distinguishing temporal anomalies with different kinds of climatological norm
====
...
variables:
  ...
  float tas(time,latitude,longitude);
    ...
    tas:units_metadata="temperature: on_scale";
    tas:cell_methods="time: maximum";
  float climatological_tas(climmonth,latitude,longitude);
    climatological_tas:standard_name="air_temperature";
    climatological_tas:units="degC";
    climatological_tas:units_metadata="temperature: on_scale";
    climatological_tas:cell_methods="climmonth: mean within years climmonth: mean over years";
  double time(time);
    time:standard_name="time";
    time:units="days since 2023-7-16";
    time:bounds="time_bounds";
    time:calendar="standard";
  double time_bounds(time,two);
  double climmonth(climmonth);
    climmonth:standard_name="time";
    climmonth:units="days since 1990-1-1";
    climmonth:climatology="climmonth_bounds";
    climmonth:calendar="standard";
  double climmonth_bounds(climmonth,two);
data:
  time_bounds=0,1, 1,2, 2,3, 3,4;
  climmonth_bounds=181,10773; // 1990-7-1, 2019-8-1, i.e. July climatology
----
Since **`climmonth`** dimension has size of unity, it could be omitted, with the **`climmonth`** variable being identified as a scalar coordinate variable in the **`coordinates`** attribute of **`climatological_tas`**.
...
[[anomalies-norm-metadata, Section 7.5.4, "Anomalies with respect to a norm metadata variable"]]
==== Anomalies with respect to a norm metadata variable

If the data variable containing the norm __N__ is not present in the dataset, the information required to define the normal statistic can instead by provided by a norm metadata variable.
...
The use of norm metadata variables is illustrated by <<reference-epoch-distinguishing-ex>>, <<anomalies-wrt-zonal-mean-ex>> and <<anomalies-wrt-area-min-ex>>.

[[anomalies-wrt-climatology-ex]]
[caption="Example 7.18. "]
.Temporal anomalies with climatological norm metadata variables
====
This example shows how the metadata of <<reference-epoch-distinguishing-ex>> can be recorded using norm metadata variables.
The data would be the same as in <<reference-epoch-distinguishing-ex>>.
...
variables:
  float delta_tas(time,latitude,longitude);
    tas:standard_name="air_temperature";
    tas:units="degC";
    tas:units_metadata="temperature: difference";
    tas:cell_methods="time: maximum time: anomaly wrt climatological_tas";
    tas:ancillary_variables="climatological_tas";
...
----
As in <<reference-epoch-distinguishing-ex>>, the metadata of **`delta_tas`** is the same in the two cases.
They are distinguished by the **`cell_methods`** and time coordinates of **`climatological_tas`**.
====
