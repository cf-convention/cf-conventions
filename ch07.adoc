...
(e.g., for a “mean” or “variance”) should (except as noted below) be weighted by the size of the interval or the size of the domain 
...
The horizontal coordinate variables to which "**`area`**" refers are in this case not explicitly indicated in **`cell_methods`** but can be identified, if necessary, from attributes attached to the coordinate variables, scalar coordinate variables, or auxiliary coordinate variables, as described in <<coordinate-types>>.  When “area” is used to reference the horizontal coordinates, then statistics involving sums of 
...
As an example, for cell_methods=“area: mean where sea_ice”, then the data would represent a mean over only the sea ice portion of the 
