...
Except in the case where __N__ has a climatological time axis (e.g., a monthly climatology), the norm metadata variable has a dimension and coordinate variable of size one, or a scalar coordinate variable, for each anomaly axis (although usually there is only one).
...
In this case, the norm metadata variable can have more than one element.
