...
__N__ is usually either a mean over a number of entire years or a climatological mean (<<climatological-statistics>>), computed from the temporal variation of __P__.
...
It can be used only for anomalies with respect to time-means or climatological means (temporal anomalies), and the information it provides about the definition of the norm is insufficient for some use-cases.

In the remainder of this section and in the following two (<<anomalies-norm-data>> and <<anomalies-norm-metadata>>) we describe a convention for anomalies with respect to a statistical norm that can be calculated from any single dimension or combination of dimensions, including the commonest case of temporal anomalies.
...
The convention of this section provides more information than the convention of <<temporal-anomalies>> about the norm in the case of temporal anomalies.
...
Therefore we call these the __anomaly coordinate variables__.
...
For an anomaly with respect to a temporal or climatological statistic, __name__ identifies the time coordinate variable of the anomaly data variable.

The anomaly coordinate variables are not coordinate variables of  __N__, but for each of the anomaly coordinate variables __N__ has a coordinate variable or scalar coordinate variable for the same physical quantity.
We call these the corresponding variables the __norm coordinate variables__.
...
Whereas the anomaly coordinate variable(s) are usually multivalued, the norm coordinate variable(s) must be of size one, with one exception:
The norm coordinate variable of a climatological statistic is multivalued if the norm has values for several intervals of the diurnal or annual cycle e.g. climatological means for each of the four seasons.

There are two alternatives for __norm__.
If the data variable containing the norm __N__ is present in the dataset, it can be named as __norm__ in **`cell_methods`** of the anomaly variable, e.g. a data variable containing zonal means for anomalies with respect to the zonal mean.
No modification to the metadata of __N__ is required for it to serve as the norm for an anomaly data variable, and it may still be treated as a data variable in its own right as well.
We describe the use of a norm data variable first (<<anomalies-norm-data>>), although it is less common, because it is conceptually more obvious.

The second alternative is for __norm__ is **`cell_methods`** __norm metadata variable__ instead of the norm data variable.
If the norm is a climatological statistic

 described in  and <<anomalies-norm-metadata>>.
...
Example 7.15 shows how the **`cell_methods`** of the norm data variable clarifies the ambiguity in the convention of Example 7.19 illustrated in Example 7.20.
...
This variable may be either a scalar coordinate variable, as in Example 7.19, or a coordinate variable with a single size-one dimension.
...
In Example 7.20, we illustrate the ambiguity in this respect of Example 7.19.
...
These two possibilities, and others, cannot be distinguished with this convention, but they can be distinguished by the conventions of <<anomalies-norm-data>> and <<anomalies-norm-metadata>>.
====
