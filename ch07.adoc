...
A norm metadata variable contains no data of its own.
...
For example, the second method must be used for a multivalued timeseries of hourly mean anomalies with respect to a climatological hourly mean diurnal cycle, because each anomaly value can refer to a different norm value (the one for the appropriate hour) i.e. the norm has a multivalued dimension.
Moreover, the anomaly time dimension may be larger than the norm climatological time dimension.
In this example, the anomaly timeseries may be several days long, while the norm time dimension spans only one climatological day.
...
Although it is formally a data variable, the norm metadata variable is recommended not to have any of the attributes of <<attribute-appendix>> except for **`cell_methods`**, which is mandatory, and **`coordinates`**, which is optional.
...
In the case where __N__ depends on multivalued climatological time, the norm metadata variable has, as its sole dimension, the dimension of the anomaly coordinate variable of time.
In this case, the norm metadata variable has more than one element.
In all other cases, the norm metadata variable has a single element.
In all cases, the data of the norm metadata variable are immaterial and of arbitrary type.
In all cases, the norm coordinate variables must have boundary variables that indicate the coordinate ranges over which __N__ was calculated from __P__.

The norm metadata variable must have a **`cell_methods`** attribute (<<cell-methods>>) with a __single__ entry, which refers to the dimension of the norm metadata variable, or the scalar norm coordinate variable, or to the combination of these if more than one.
The **`cell_methods`** attribute indicates how __N__ was computed from the variation of __P__, as in <<anomalies-norm-data>>.
...
In this case, the norm metadata variable is one-dimensional (not scalar, as in all other cases), with the dimension of the anomaly coordinate variable of time (6 in the example); as usual, its data type and values are immaterial because they are not used for anything.
It has an auxiliary coordinate variable, with the same dimension as itself, named by its **`coordinates`** attribute, but it has no scalar coordinate variables.
The auxiliary coordinate variable contains a list of indices to the climatological time coordinate variable of the norm data variable __N__.
...
The auxiliary coordinate variable has a **`compress`** attribute, which contains the name of the climatological time coordinate variable of __N__, in order to make the link between them.

Element __i__ of the auxiliary coordinate variable contains a value, say __j__, which is the index (numbering from 0) of the element of the climatological time dimension of __N__ that corresponds to element __i__ of the anomaly time dimension of __A__.
This indicates that the slice of the anomaly data variable __A__ with index __i__ along this dimension is the difference between the corresponding slice of the original data variable __P__ and the slice of the norm data variable __N__ with index __j__ along its climatological time dimension, whose climatological time is given by element __j__ of the climatological time coordinate variable.
In an abstract sense, the norm metadata variable suggests that __A__ is the difference between __P__ and __N'__, where __N'__ has all same dimensions as __A__ and __P__, including time, and is constructed by selecting, with repetition if necessary, and then concatenating the slices of __N__ whose climatological time indices are listed in the auxiliary coordinate variable.
