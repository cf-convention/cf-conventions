...
In a data variable containing anomaly data for some quantity, each element __A__ is the difference __P__ - __N__ between a particular value __P__ of the quantity and a normal value or norm __N__ of the same quantity.
__N__ is some statistic calculated from the values of the quantity that lie within specified ranges of one or more of its coordinates.
 __P__ can be, but is not necessarily, one of the set of values from which __N__ is calculated.

A data variable __A__ containing anomalies with respect to a norm is __notionally__ the difference between a data variable __P__ containing the original data and a data variable __N__ containing the statistical norm of the quantity.
...
The data variables __P__ and __N__ are usually not actually present in the dataset.

The commonest kind of anomaly is the difference between the value __P__ of a quantity and the mean __N__ of the same quantity over some range of time coordinates, usually called the "climatological normal", the "climate normal", or the "climatology".
__N__ is usually either a mean over a number of entire years or a climatological mean (Section 7.4, "Climatological statistics").
The time coordinate of the anomaly may or may not lie within the range of times from which __N__ is calculated.

In the case of an anomaly __A__ with respect to a climatology, __N__ contains a time-mean or a climatological mean.
__N__ has all the same dimensions and coordinate variables as __A__ except for time.
__N__ is computed from the temporal variation of __P__.

CF offers two conventions for describing anomaly data.
The convention of Section 7.5.1 depends on special standard names and can be used only for anomalies with respect to climatology (temporal anomalies).
It is simple but the information it provides about the definition of the norm is insufficient for some use-cases.
The convention of Section 7.5.2 can be used for anomalies with respect to any dimension and provides more information about the norm, but is consequently more complicated.


[[temporal-anomalies, Section 7.5.1, "Temporal anomalies using **`anomaly`** standard names"]]
==== Temporal anomalies using **`anomaly`** standard names

Several CF standard names (ending with **`_anomaly`**) have been defined for anomalies with respect to a climatological normal.
The start and end of the climatological normal period may be recorded in the bounds of a variable having **`reference_epoch`** as its standard name attribute.
This variable may be either a scalar coordinate variable, as in Example 7.A, or a coordinate variable with a single size-one dimension.
...
The data variable **`delta_tas`** contains daily maximum temperatures for 16th-19th July 2023 expressed as anomalies with respect to the climatological normal time-mean of 1990-2019, which is defined by the bounds of **`reference_epoch`**.
Note that 10957 days since 1st January 1990 in the **`standard`** calendar is 1st January 2020.
The single value of the **`reference_epoch`** coordinate variable should be a representative time within the climatological interval (see Section 7.4).
...
The convention with **`_anomaly`** standard names and **`reference_epoch`** gives no indication of the **`cell_methods`** describing the climatological normal.
The climatological normal variable is not necessarily present in the file, and even if it is present, the convention does not provide any link between the anomaly variable and the climatological normal variable.
Hence the interpretation of the anomaly is unclear.
In Example 7.B, we illustrate the ambiguity of Example 7.A in this respect.

---

_Example 7.B. Ambiguity in interpreting an anomaly data variable with a reference epoch._

In Example 7.A, **`delta_tas`** (__A__ in the notation at the start of this section) is interpreted as the difference between two other variables (__P__ and __N__), which may or may not be contained in the dataset e.g. **`delta_tas`** (__A__) = **`tas`** (__P__) - **`climatological_tas`** (__N__), where __N__ isn't fully described by the metadata.
...
with dimension **`climonths=12`** for climatological monthly means.
These two possibilities, and others, cannot be distinguished with this convention, but they can be distinguished by the convention of Section 7.5.2.

---

### 7.5.2. Convention describing the norm for general anomalies

The convention of this subsection can be used to describe anomalies with respect to a statistical norm calculated from any single dimension or combination of dimensions, including the commonest case of temporal anomalies (i.e. with respect to a climatological norm, as in Section 7.5.1).
...
This convention also allows us to clarify the definition of the climatological norm in the case of temporal anomalies (removing the ambiguity of Example 7.B).

A data variable is described as an anomaly using this convention by including in its **`cell_methods`** attribute a string of the form "_name_**`:`** [_name_**`:`** ...] **`anomaly wrt`** __norm__".
...
* that __N__ was calculated from the variation of __P__ over dimensions identified by the _name_(s).

Each _name_ must be the name of a coordinate variable (and dimension) or a scalar coordinate variable of the anomaly data variable __A__.
Therefore we call these the _anomaly coordinate variables_.
They are not coordinate variables of  __N__, which is not necessarily present in the dataset, but __N__ has coordinate variables for the same quantities as the anomaly coordinate variables.
Usually there is only one _name_ and one anomaly coordinate variable, and usually it is a spatiotemporal coordinate variable.

For a data variable containing anomalies with respect to climatology, _name_ identifies the time coordinate or scalar coordinate variable as the anomaly coordinate e.g. "**`time: anomaly wrt`** __norm__".
For an anomaly with respect to the zonal mean, the anomaly coordinate is longitude.
For an anomaly with respect to the area minimum, we need two names e.g. "**`lat: lon: anomaly wrt`** __norm__".
As described in Section 7.3.1, the combination of horizontal coordinate variables can alternatively be represented by the word **`area`**, thus "**`area: anomaly wrt`** __norm__".

There are two alternatives for __norm__, as follows:

(1) **Norm data variable**.
If the data variable containing the norm __N__ is present in the dataset, __norm__ is the name of that variable, e.g. a data variable containing zonal means for anomalies with respect to the zonal mean.
If the anomaly data variable has a **`standard_name`** attribute, it must _not_ be a standard name ending in **`_anomaly`**, and the __norm__ variable must have the same attribute.

The __norm__ variable must have all the same coordinate variables (and hence dimensions) and scalar coordinate variables as the anomaly data variable, _except_ for the anomaly coordinate variables.
Corresponding to each of the anomaly coordinate variables (usually there is only one), the __norm__ variable must have a coordinate variable or scalar coordinate variable for the same physical quantity as the anomaly coordinate variable.
We call these the _norm coordinate variables_ (usually there is only one).
For instance, a norm data variable containing zonal means has a norm coordinate variable for longitude, corresponding to the anomaly coordinate variable for longitude of the anomaly data variable.

Whereas the anomaly coordinate variable(s) are usually multivalued, the norm coordinate variable(s) must be of size one (except for norms which are climatological statistics; see paragraph below).
This is because the norm is a statistic that represents the variation of __P__ within the range of the norm coordinate variable.
...
The norm coordinate variables must have boundary variables to indicate the coordinate ranges over which __N__ was calculated from __P__.

A norm data variable for a climatological statistic (in the sense of Section 7.4) has a norm coordinate variable which

* has a **`cell_methods`** entry containing **`within`** and **`over`** keywords,

* may have a dimension greater than one (for climatological variation **`within days`** or **`within years`**), and

* must have a **`climatology`** attribute to identify its boundary variable.

Example 7.C shows how the **`cell_methods`** of the norm data variable clarifies the ambiguity in the convention of Section 7.5.1 illustrated in Example 7.B.

---

_Example 7.C. Distinguishing temporal anomalies with different kinds of climatological norm_
...
It is the difference between the daily maxima contained in data variable **`tas`**, and the time-mean  **`climatological_tas`** of the entire 30-year period 1990-2019:
...
variables:
  float delta_tas(time,latitude,longitude);
    tas:standard_name="air_temperature";
    tas:units="degC";
    tas:cell_methods="time: maximum time: anomaly wrt climatological_tas";
  float tas(time,latitude,longitude);
    tas:standard_name="air_temperature";
    tas:units="degC";
    tas:cell_methods="time: maximum";
...
The **`cell_methods`** of **`delta_tas`** records that daily maxima were first calculated, then anomalised with respect to the climatological mean.
...
----
  ...
  double time(time);
    ...
    time:calendar="standard";
  double time_bounds(time,two);
  double climmonths(climmonths);
    climmonths:standard_name="time";
    climmonths:units="days since 1990-1-1";
    climmonths:climatology="climmonths_bounds";
    climmonths:calendar="standard";
  double climmonths_bounds(climmonths,two);
data:
  time_bounds=0,1, 1,2, 2,3, 3,4;
  climmonths_bounds=0,10623, ... // 1990-1-1, 2019-2-1, i.e. January climatology
    181,10773, ... // 1990-7-1, 2019-8-1, i.e. July climatology
    334,10957; // 1990-12-1, 2020-1-1, i.e. December climatology
----

In this example, **`climatological_tas`** contains climatological monthly means for every month.
Since the **`time`** coordinates all lie within July, it would be sufficient to provide the climatological mean for July only.
In that case the **`climmonths`** variable would have a dimension **`climmonths=1`**, or it could be a scalar coordinate variable.

The metadata of **`delta_tas`** is the same for the two examples above.
They are distinguished by the **`cell_methods`** and time coordinates of **`climatological_tas`**.

---

(2) **Norm metadata variable**. If the data variable containing the norm __N__ is not present in the dataset, the information required to define the normal statistic can instead by provided by a norm metadata variable.

For each of the anomaly coordinate variables, the norm metadata variable must have a coordinate variable or scalar coordinate variable for the same physical quantity.
These are exactly the same the norm coordinate variables (usually there is only one of them) as in case (1), and they must have the same sizes and bounds as in case (1).
The norm metadata variable must have a **`cell_methods`** attribute with a _single_ entry, which refers to the norm coordinate variable, or the combination of them if more than one, to indicate how __N__ was computed from the variation of __P__, as in case (1).

The norm metadata variable must have no coordinate or scalar coordinate variables other than the norm coordinate variables.
If the sole, or every, norm coordinate variable is a scalar coordinate variable, the norm metadata variable will have no dimensions i.e.
it will be a scalar.
Except if there is a multivalued norm coordinate variable for climatological time, the norm metadata variable will have only one element.
The data in the norm metadata variable is immaterial and of arbitrary type, because the purpose of the variable is only to host the norm coordinate variables and the **`cell_methods`**.

Although it is formally a data variable, it is recommended that a norm metadata variable does not have any of the attributes of Appendix A except for **`cell_methods`** and any which are marked as applicable to both data and domain variables.

The use of norm metadata variables is illustrated by Examples 7.D, 7.E and 7.F.

---

_Example 7.D. Temporal anomalies with climatological norm metadata variables_

This example shows how the metadata of Example 7.C can be recorded using norm metadata variables.
The data would be the same as in Example 7.C.
...
----
  float delta_tas(time,latitude,longitude);
    ...
    tas:units="degC";
    tas:cell_methods="time: maximum time: anomaly wrt climatological_tas";
  float climatological_tas(climmonths);
    climatological_tas:cell_methods="climmonths: mean within years climmonths: mean over years";
  double time(time);
    time:standard_name="time";
    time:units="days since 2023-7-16";
...
As in Example 7.C, the metadata of **`delta_tas`** is the same in the two cases.
They are distinguished by the **`cell_methods`** and time coordinates of **`climatological_tas`**.

---

_Example 7.E. Anomalies with respect to a zonal mean_
...
_Example 7.F. Anomalies with respect to the minimum within a horizontal area_
