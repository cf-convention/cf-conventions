...
For example, a commonly-used anomaly is the temperature at a specific time and place minus the long-term global average temperature.

CF offers two conventions for describing anomaly data.
In <<temporal-anomalies>>, we describe a simple convention that depends on special standard names.
It can only be used for simple temporal anomalies, and is insufficient for some use-cases.
In the remainder of this section and the following two (<<anomalies-norm-data>> and <<anomalies-norm-metadata>>), we describe two more general conventions for anomalies that can handle more complex use-cases.

The generalized definition of an anomaly value __A__ of some physical quantity __q__ is the difference __P__ &minus; __N__ between a particular value __P__ of __q__ and a normal value or norm __N__ of __q__.
...
__P__ has all the same netCDF dimensions and coordinate variables as __A__, but __N__ shares only a subset of the dimensions and coordinate variables of __A__.
Corresponding to each of the dimensions of __A__ which __N__ does not share, it has its own dimension for the same spatiotemporal coordinate or other quantity.
These dimensions, which __N__ doesn't share with __A__ and __P__, are the ones over which the normal statistic is calculated.

The commonest kind of anomaly __A__ is a temporal anomaly.
...
In this section and the following two (<<anomalies-norm-data>> and <<anomalies-norm-metadata>>) we describe a general convention for anomalies with respect to a statistical norm calculated from any single dimension or combination of dimensions.
In this convention the norm statistic does not have to be a mean.
...
When temporal anomalies are described following the convention of this section, more information can be recorded about the norm than when following the alternative, less general, convention of <<temporal-anomalies>>.
...
* that the data variable contains anomalies (__A__ in the notation of the start of this section) calculated from data (__P__) with the same dimensions, as the difference from the normal statistic named __norm__ (__N__).

* at which stage the anomaly __A__ = __P__ &minus; __N__ was calculated.
...
Each __name__ must be the name of an "axis" of the anomaly data variable __A__.
We call these the "anomaly axes".
By "axis" we mean a dimension and its corresponding coordinate variable, or a scalar coordinate variable.
In either case, we refer to the variable as an ``anomaly coordinate variable''.
...
For instance, for a data variable containing anomalies with respect to the zonal mean, __name__ identifies longitude as the anomaly axis e.g. "**`longitude: anomaly wrt`** __norm__".
For an anomaly with respect to the area minimum, we need two __name__s e.g. "**`lat: lon: anomaly wrt`** __norm__".
As described in <<cell-methods>>, the combination of horizontal axes can alternatively be represented by the word **`area`**, thus "**`area: anomaly wrt`** __norm__".
...
possible deletion ****
For each anomaly axis of __A__ (although usually there is only one), __N__ has an axis, called a "norm axis", with a coordinate variable (and dimension of the same name) or scalar coordinate variable.
This variable is called the "norm coordinate variable", and it must have the same **`standard_name`** as the anomaly coordinate variable.
The norm coordinate variable indicates the range(s) of coordinates over which the statistic __N__ was calculated from the variation of __P__.
For instance, if __N__ contains zonal means, it has a norm coordinate variable for longitude that indicates the range of longitudes over which the zonal mean was calculated.
possible deletion ****
...
For example, the second method must be used for a timeseries of hourly mean anomalies with respect to a climatological hourly mean diurnal cycle, because the norm axis is multivalued and each anomaly value is relative to a different norm value (the one for the appropriate hour).
...
For each anomaly axis (although usually there is only one), the norm data variable has an axis, called a "norm axis", with a coordinate variable (and dimension of the same name) or a scalar coordinate variable (named in its **`coordinates`** attribute).
In either case, we refer to it as a ``norm coordinate variable''.
The normal coordinate variable must have the same **`standard_name`** as the anomaly coordinate variable.
...
Any norm axis dimensions must be included among the dimensions of the anomaly data variable as well as the norm data variable; likewise, any scalar norm coordinate variables must be named in the **`coordinates`** attribute of the anomaly data variable as well as the norm data variable.
...
====
The anomaly data variable **`delta_tas(time,latitude,longitude)`** contains daily maxima (along the **`time`** dimension) for 16th-19th July 2023 of the anomaly in **`air_temperature`** with respect to the climatological mean of 1990-2019.
...
The norm metadata data variable is of arbitrary type because it contains no meaningful data.
It is a "dummy" variable whose data values are immaterial, like a grid mapping variable, for instance.

Although it is formally a data variable, CF recommends that the norm metadata variable not have any of the attributes of <<attribute-appendix>> other than **`cell_methods`** and **`coordinates`**.
Attributes describing the __norm__ quantity (standard name, units, etc.) are unnecessary, because they must be the same as for the anomaly data variable.
...
For each anomaly axis (although usually there is only one), the norm metadata variable has an norm axis, with either a coordinate variable (and dimension of the same name) or a scalar coordinate variable.
This norm coordinate variable must have the same **`standard_name`** as the anomaly coordinate variable.
If it is a coordinate variable with a dimension, the dimension must be of size one, except in the case where __N__ has a climatological time axis as described in <<climatological-statistics>> (e.g., a monthly climatology).
...
The norm metadata variable must have no coordinate variables other than the norm coordinate variables.

Except in the case where __N__ depends on multivalued climatological time, for each of the anomaly coordinate variables (usually there is only one), the norm metadata variable must have a scalar or a size-one norm coordinate variable with the same **`standard_name`** as the anomaly coordinate variable.
Any scalar norm coordinate variables must be named in the **`coordinates`** attribute of the both the norm metadata variable and the anomaly data variable.
Any size-one dimensions of the norm metadata variable must also be dimensions of the anomaly data variable.
...
In the case where __N__ has a multivalued climatological time axis (such as those illustrated in Examples 7.9, 7.10, and 7.11), the norm metadata variable has, as its sole dimension, the corresponding climatological time dimension.
In this case, the norm metadata variable has more than one element; in all others, it has only a single element.
...
The norm metadata variable must have a **`cell_methods`** attribute (<<cell-methods>>) with a __single__ entry, indicating how __N__ was computed from the variation of _P_, as in <<anomalies-norm-data>>.
The **`cell_methods`** must refer to the norm coordinate variable (or combination thereof, if more than one).
...
data:
  zmlongitude=-165; // a value within the range; it's usually arbitrary
  zmbounds=-240,-90;
----
...
The mapping between the time axis of the anomaly variable and the climatological time axis of the norm is recorded by an auxiliary coordinate variable named by the **`coordinates`** attribute of the norm metadata variable.
This auxiliary coordinate variable is one-dimensional and has the same time dimension as the anomaly data variable.
...
The auxiliary coordinate variable has a **`compress`** attribute naming the climatological time axis, in order to make the link between them.
...
Suppose that __A__ contains monthly anomalies for the months of June, July, and August of 2023 and 2024 with respect to __N__, the 30-year climatological monthly means for 1990&ndash;2019.
...
It would also be valid to use compression by gathering and provide only the climatology for June, July, and August, in which case the climatological time axis would have a dimension of 3.
...
This means that element **0** of **`time`**, which is 2023-06-16 (with bounds of 2023-06-01 and 2023-07-01, i.e. the whole of June 2023) is paired with element **5** of **`climatological_time`**, which is the climatology for June 1991&ndash;2000.
...
This convention using **`_anomaly`** standard names and **`reference_epoch`** is restricted to those common cases that have such standard names defined.
...
====
The standard name **`air_temperature_anomaly`** of Example 7.20 is defined as the difference of air temperature from its climatology, i.e., **`delta_tas`** (__A__) = **`tas`** (__P__) &minus; **`climatological_tas`** (__N__).
 __P__ and __N__ may or may not be contained in the dataset, but __N__ is not fully described by the metadata.

One possibility is that **`delta_tas`** is the difference between daily maxima in **`tas`** and the time-mean of the entire 30-year period 1990&ndash;2019 in **`climatological_tas`**:
----
  float tas(time,latitude,longitude);
    tas:standard_name="air_temperature";
    tas:units="degC";
